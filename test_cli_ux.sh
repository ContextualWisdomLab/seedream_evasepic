...
# Also verify that a missing audio_path takes precedence over an invalid model
if ! bash "$SCRIPT_DIR/transcribe.sh" "" "invalid_model" 2>&1 | grep -q "Error: Missing required argument: <audio_path>"; then
  echo "FAIL: transcribe.sh did not prioritize missing audio_path over invalid model" >&2
  exit 1
fi
