...
echo "dummy" > "$TMP_TEST_DIR/dummy.mp4"
if ! FFMPEG="/bin/echo" FFPROBE="/nonexistent/ffprobe" bash "$SCRIPT_DIR/extract-frames.sh" "$TMP_TEST_DIR/dummy.mp4" "$TMP_TEST_DIR" 2>&1 | grep -q "Error: ffprobe not found"; then
