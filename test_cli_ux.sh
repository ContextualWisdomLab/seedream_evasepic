...
echo "=== Testing yt-dlp concurrent fragments ==="
if ! grep -q "\-\-concurrent-fragments 4" "$SCRIPT_DIR/download-reference.sh"; then
  echo "FAIL: download-reference.sh does not include --concurrent-fragments 4 for yt-dlp" >&2
  exit 1
fi
echo "PASS: download-reference.sh includes --concurrent-fragments 4"
echo "====================================="
