...
if ! bash "$SCRIPT_DIR/extract-frames.sh" "dummy_path" 2>&1 | grep -q "Error: Missing required argument: <output_dir>"; then
  echo "FAIL: extract-frames.sh did not print explicit error message for output_dir" >&2
  exit 1
fi
echo "PASS: extract-frames.sh prints explicit error message"
...
echo "PASS: transcribe.sh prints explicit error message"
echo "====================================="
