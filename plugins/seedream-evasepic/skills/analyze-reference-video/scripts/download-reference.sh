...
# UX Enhancement: Format file size into a human-readable format (B/KiB/MiB/GiB) using POSIX awk
HUMAN_READABLE_SIZE="$(awk -v bytes="$FILE_SIZE_BYTES" 'BEGIN {
  ...
  if (idx == 1) printf "%d %s", bytes, units[idx]
  else printf "%.2f %s", bytes, units[idx]
}')"
