...
HUMAN_READABLE_SIZE="$(awk -v size="$FILE_SIZE_BYTES" 'BEGIN { split("B KiB MiB GiB TiB", u, " "); i=1; while(size>=1024 && i<5){size/=1024; i++} if(i==1) printf "%d %s", size, u[i]; else printf "%.2f %s", size, u[i] }')"
