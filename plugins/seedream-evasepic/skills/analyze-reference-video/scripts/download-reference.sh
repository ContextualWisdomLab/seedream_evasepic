...
if ! command -v yt-dlp >/dev/null 2>&1; then
  ...
  if ! command -v yt-dlp >/dev/null 2>&1; then
    printf "%b\n" "${RED}Error: yt-dlp was installed but is not accessible in your PATH.${NC}" >&2
    printf "%b\n" "${CYAN}Please fix your PATH or install manually:${NC}" >&2
    printf "%b\n" "${CYAN}  export PATH=\"\$HOME/.local/bin:\$PATH\"${NC}" >&2
    exit 1
  fi
fi
