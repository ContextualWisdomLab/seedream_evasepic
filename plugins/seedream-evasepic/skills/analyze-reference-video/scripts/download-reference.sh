...
if [ "$FILE_SIZE_BYTES" -lt 1024 ]; then
  FORMATTED_SIZE="${FILE_SIZE_BYTES} B"
elif [ "$FILE_SIZE_BYTES" -lt 1048576 ]; then
  FORMATTED_SIZE="$(awk -v b="$FILE_SIZE_BYTES" 'BEGIN { printf "%.2f KiB", b / 1024 }')"
elif [ "$FILE_SIZE_BYTES" -lt 1073741824 ]; then
  FORMATTED_SIZE="$(awk -v b="$FILE_SIZE_BYTES" 'BEGIN { printf "%.2f MiB", b / 1048576 }')"
else
  FORMATTED_SIZE="$(awk -v b="$FILE_SIZE_BYTES" 'BEGIN { printf "%.2f GiB", b / 1073741824 }')"
fi
terminal_print_value "${CYAN}Size: " "${FORMATTED_SIZE}" "${NC}"
