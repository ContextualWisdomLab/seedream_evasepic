...
# Optimization: Early return if target file already exists and is non-empty
# This avoids expensive redundant network requests and dependency checks.
if [ -f "$OUTPUT" ] && [ -s "$OUTPUT" ]; then
  printf "%b\n" "${GREEN}File already exists and is not empty, skipping download: ${NC}$OUTPUT"
  exit 0
fi
