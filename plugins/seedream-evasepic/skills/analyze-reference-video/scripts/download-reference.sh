...
if [ "$FILE_SIZE_BYTES" -ge 1073741824 ]; then
  FILE_SIZE_HUMAN="$(awk -v size="$FILE_SIZE_BYTES" 'BEGIN { printf "%.2f GiB", size / 1073741824 }')"
elif [ "$FILE_SIZE_BYTES" -ge 1048576 ]; then
  FILE_SIZE_HUMAN="$(awk -v size="$FILE_SIZE_BYTES" 'BEGIN { printf "%.2f MiB", size / 1048576 }')"
elif [ "$FILE_SIZE_BYTES" -ge 1024 ]; then
  FILE_SIZE_HUMAN="$(awk -v size="$FILE_SIZE_BYTES" 'BEGIN { printf "%.2f KiB", size / 1024 }')"
else
