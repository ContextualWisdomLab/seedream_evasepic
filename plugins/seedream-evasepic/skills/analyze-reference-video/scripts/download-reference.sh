...
if [[ "$FILE_SIZE_BYTES" =~ ^[0-9]+$ ]]; then
  if [ "$FILE_SIZE_BYTES" -ge 1073741824 ]; then
    FORMATTED_SIZE="$(awk -v size="$FILE_SIZE_BYTES" 'BEGIN { printf "%.2f GB", size / 1073741824 }')"
  elif [ "$FILE_SIZE_BYTES" -ge 1048576 ]; then
    FORMATTED_SIZE="$(awk -v size="$FILE_SIZE_BYTES" 'BEGIN { printf "%.2f MB", size / 1048576 }')"
  elif [ "$FILE_SIZE_BYTES" -ge 1024 ]; then
    FORMATTED_SIZE="$(awk -v size="$FILE_SIZE_BYTES" 'BEGIN { printf "%.2f KB", size / 1024 }')"
  else
