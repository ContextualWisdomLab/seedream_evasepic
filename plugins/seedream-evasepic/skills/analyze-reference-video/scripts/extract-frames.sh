...
# ⚡ Bolt Optimization: Batch ffprobe queries into a single invocation to minimize process startup overhead.
RAW_PROBE=$("$FFPROBE" -v error -select_streams v:0 \
  -show_entries format=duration:stream=width,height,r_frame_rate \
  -of default=noprint_wrappers=1:nokey=0 "$VIDEO" 2>/dev/null || echo "")

DURATION=$(echo "$RAW_PROBE" | awk -F= '/^duration=/ {print $2}')
WIDTH=$(echo "$RAW_PROBE" | awk -F= '/^width=/ {print $2}')
HEIGHT=$(echo "$RAW_PROBE" | awk -F= '/^height=/ {print $2}')
FPS=$(echo "$RAW_PROBE" | awk -F= '/^r_frame_rate=/ {print $2}')

[ -n "$WIDTH" ] && [ -n "$HEIGHT" ] && RESOLUTION="${WIDTH}x${HEIGHT}" || RESOLUTION="unknown"
