...
# ⚡ Bolt Optimization: Batch metadata extraction into a single ffprobe call to reduce I/O overhead
PROBE_OUT=$("$FFPROBE" -v error -select_streams v:0 \
  -show_entries format=duration:stream=width,height,r_frame_rate \
  -of default=noprint_wrappers=1:nokey=0 "$VIDEO" 2>/dev/null || true)

DURATION_VAL=$(echo "$PROBE_OUT" | grep "^duration=" | cut -d'=' -f2 || true)
DURATION=${DURATION_VAL:-0}

WIDTH=$(echo "$PROBE_OUT" | grep "^width=" | cut -d'=' -f2 || true)
HEIGHT=$(echo "$PROBE_OUT" | grep "^height=" | cut -d'=' -f2 || true)
...
FPS_VAL=$(echo "$PROBE_OUT" | grep "^r_frame_rate=" | cut -d'=' -f2 || true)
FPS=${FPS_VAL:-unknown}
