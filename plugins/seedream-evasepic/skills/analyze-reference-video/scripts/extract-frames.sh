...
# Optimization: Combine multiple ffprobe queries into a single invocation to minimize process startup overhead
PROBE_OUTPUT=$("$FFPROBE" -v error -select_streams v:0 \
  -show_entries format=duration:stream=width,height,r_frame_rate \
  -of default=noprint_wrappers=1:nokey=0 "$VIDEO" 2>/dev/null || true)

DURATION=$(echo "$PROBE_OUTPUT" | awk -F= '$1=="duration"{print $2}' | head -n1)
DURATION=${DURATION:-0}

WIDTH=$(echo "$PROBE_OUTPUT" | awk -F= '$1=="width"{print $2}' | head -n1)
HEIGHT=$(echo "$PROBE_OUTPUT" | awk -F= '$1=="height"{print $2}' | head -n1)
...
FPS=$(echo "$PROBE_OUTPUT" | awk -F= '$1=="r_frame_rate"{print $2}' | head -n1)
FPS=${FPS:-unknown}
