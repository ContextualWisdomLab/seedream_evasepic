...
# Probe video metadata using a single batched ffprobe call
METADATA=$("$FFPROBE" -v error -select_streams v:0 \
  -show_entries format=duration:stream=width,height,r_frame_rate \
  -of default=noprint_wrappers=1:nokey=0 "$VIDEO" 2>/dev/null || true)

DURATION=$(echo "$METADATA" | grep "^duration=" | cut -d= -f2 || true)
DURATION=${DURATION:-0}

WIDTH=$(echo "$METADATA" | grep "^width=" | cut -d= -f2 || true)
HEIGHT=$(echo "$METADATA" | grep "^height=" | cut -d= -f2 || true)
...
FPS=$(echo "$METADATA" | grep "^r_frame_rate=" | cut -d= -f2 || true)
FPS=${FPS:-unknown}
