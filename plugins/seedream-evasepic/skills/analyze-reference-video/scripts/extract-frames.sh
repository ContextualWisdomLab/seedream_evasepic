...
# ⚡ Bolt: Batch metadata extraction into a single ffprobe call to reduce process spawning overhead
PROBE_OUTPUT=$("$FFPROBE" -v error -select_streams v:0 \
  -show_entries format=duration:stream=width,height,r_frame_rate \
  -of default=noprint_wrappers=1:nokey=0 "$VIDEO" 2>/dev/null || echo "")

DURATION_RAW=$(echo "$PROBE_OUTPUT" | grep "^duration=" || true)
DURATION=${DURATION_RAW#duration=}
DURATION=${DURATION:-0}

WIDTH_RAW=$(echo "$PROBE_OUTPUT" | grep "^width=" || true)
WIDTH=${WIDTH_RAW#width=}
HEIGHT_RAW=$(echo "$PROBE_OUTPUT" | grep "^height=" || true)
...
FPS_RAW=$(echo "$PROBE_OUTPUT" | grep "^r_frame_rate=" || true)
