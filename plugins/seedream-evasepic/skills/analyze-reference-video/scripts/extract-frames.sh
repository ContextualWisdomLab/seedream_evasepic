...
# ⚡ Bolt Optimization: Consolidate multiple ffprobe calls into a single invocation
# to avoid significant process startup overhead and speed up metadata extraction.
PROBE_DATA=$("$FFPROBE" -v error -select_streams v:0 \
  -show_entries format=duration:stream=width,height,r_frame_rate \
  -of default=noprint_wrappers=1:nokey=0 "$VIDEO" 2>/dev/null || echo "")

DURATION=$(echo "$PROBE_DATA" | awk -F= '/^duration=/ {print $2}')
DURATION=${DURATION:-0}

WIDTH=$(echo "$PROBE_DATA" | awk -F= '/^width=/ {print $2}')
HEIGHT=$(echo "$PROBE_DATA" | awk -F= '/^height=/ {print $2}')
...
FPS=$(echo "$PROBE_DATA" | awk -F= '/^r_frame_rate=/ {print $2}')
FPS=${FPS:-unknown}
