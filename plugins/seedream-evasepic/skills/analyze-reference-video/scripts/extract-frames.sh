...
safe_dur="$(terminal_safe_text "$DURATION")"
safe_res="$(terminal_safe_text "$RESOLUTION")"
safe_fps="$(terminal_safe_text "$FPS")"
printf "%b%s%b%s%b%s\n" "${CYAN}Duration: ${NC}" "${safe_dur}s" " | ${CYAN}Resolution: ${NC}" "$safe_res" " | ${CYAN}FPS: ${NC}" "$safe_fps"
