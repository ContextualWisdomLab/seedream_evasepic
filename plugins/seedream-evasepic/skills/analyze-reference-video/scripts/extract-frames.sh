...
# Probe video metadata using a single ffprobe call to reduce process spawning and I/O overhead
PROBE_DATA=$("$FFPROBE" -v error -select_streams v:0 -show_entries format=duration:stream=width,height,r_frame_rate -of default=noprint_wrappers=1:nokey=0 "$VIDEO" 2>/dev/null || true)

DURATION_RAW=$(echo "$PROBE_DATA" | grep '^duration=' || true)
DURATION=${DURATION_RAW#duration=}
DURATION=${DURATION:-0}

WIDTH_RAW=$(echo "$PROBE_DATA" | grep '^width=' | head -n1 || true)
WIDTH=${WIDTH_RAW#width=}

HEIGHT_RAW=$(echo "$PROBE_DATA" | grep '^height=' | head -n1 || true)
...
FPS_RAW=$(echo "$PROBE_DATA" | grep '^r_frame_rate=' | head -n1 || true)
FPS=${FPS_RAW#r_frame_rate=}
FPS=${FPS:-unknown}
