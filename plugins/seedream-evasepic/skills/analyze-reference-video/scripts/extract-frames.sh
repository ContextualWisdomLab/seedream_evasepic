...
DURATION_RAW=$(echo "$METADATA" | grep "^duration=" || true)
DURATION=${DURATION_RAW#duration=}
DURATION=${DURATION:-0}

WIDTH_RAW=$(echo "$METADATA" | grep "^width=" || true)
WIDTH=${WIDTH_RAW#width=}
HEIGHT_RAW=$(echo "$METADATA" | grep "^height=" || true)
...
FPS_RAW=$(echo "$METADATA" | grep "^r_frame_rate=" || true)
