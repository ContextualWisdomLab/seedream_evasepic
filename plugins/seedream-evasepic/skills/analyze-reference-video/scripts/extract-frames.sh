...
# Probe video metadata (Consolidated for performance)
# Bolt: Reduces process startup overhead by extracting all metadata in one pass
PROBE_OUT=$("$FFPROBE" -v error -select_streams v:0 -show_entries format=duration:stream=width,height,r_frame_rate -of default=noprint_wrappers=1:nokey=0 "$VIDEO" 2>/dev/null || true)

DURATION=$(echo "$PROBE_OUT" | awk -F= '$1=="duration" {print $2}')
[ -z "$DURATION" ] && DURATION=0

WIDTH=$(echo "$PROBE_OUT" | awk -F= '$1=="width" {print $2}')
HEIGHT=$(echo "$PROBE_OUT" | awk -F= '$1=="height" {print $2}')
...
FPS=$(echo "$PROBE_OUT" | awk -F= '$1=="r_frame_rate" {print $2}')
[ -z "$FPS" ] && FPS="unknown"
