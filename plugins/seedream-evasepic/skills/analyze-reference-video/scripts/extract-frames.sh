...
# ⚡ Bolt Optimization: Consolidate multiple ffprobe queries into a single invocation
# and parse with awk to significantly reduce process startup overhead.
read -r DURATION RESOLUTION FPS <<< "$("$FFPROBE" -v error -select_streams v:0 -show_entries format=duration:stream=width,height,r_frame_rate -of default=noprint_wrappers=1 "$VIDEO" 2>/dev/null | awk -F= '
{
  if($1=="duration") d=$2;
  else if($1=="width") w=$2;
  else if($1=="height") h=$2;
  else if($1=="r_frame_rate") f=$2;
} END {
  printf "%s %s %s\n", (d ? d : "0"), (w && h ? w"x"h : "unknown"), (f ? f : "unknown")
}')"
