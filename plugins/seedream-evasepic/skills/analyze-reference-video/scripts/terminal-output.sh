...
#   Replaced dynamic `for` loops calling `printf` with unrolled, sequential
#   native parameter expansions (`${value//pattern/replacement}`).
#   This avoids spawning built-ins inside a loop, significantly reducing overhead
#   and making the text neutralization up to ~5x faster.
terminal_safe_text() {
  local value="${1-}"
