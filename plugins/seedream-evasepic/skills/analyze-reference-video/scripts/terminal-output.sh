...
terminal_safe_text() {
  ...
  # Performance Optimization: C0 and C1 control character replacements are unrolled
  # into direct parameter expansions instead of using a 'for' loop with 'printf -v'.
  # This avoids the significant execution overhead of repeated built-in command invocations,
  # resulting in substantially faster string neutralization.
