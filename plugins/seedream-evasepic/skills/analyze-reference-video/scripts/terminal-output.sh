...
_init_safe_text_arrays
unset -f _init_safe_text_arrays
...
terminal_safe_text() {
  ...
  for ((i=0; i<${#_SAFE_TEXT_CONTROLS[@]}; i++)); do
    value=${value//"${_SAFE_TEXT_CONTROLS[i]}"/"${_SAFE_TEXT_REPLACEMENTS[i]}"}
  done
  value=${value//$'\177'/\\x7F}
