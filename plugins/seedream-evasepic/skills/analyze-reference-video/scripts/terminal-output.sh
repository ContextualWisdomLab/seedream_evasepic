...
# ⚡ Precompute arrays at script load time to avoid repeated printf overhead
declare -a _TERMINAL_C0_CTRL _TERMINAL_C0_REPL _TERMINAL_C1_CTRL _TERMINAL_C1_REPL
for _code in {1..31}; do
  printf -v _octal '%03o' "$_code"
  printf -v _TERMINAL_C0_CTRL[$_code] '%b' "\\${_octal}"
  printf -v _TERMINAL_C0_REPL[$_code] '\\x%02X' "$_code"
done
for _code in {128..159}; do
  printf -v _octal '%03o' "$_code"
  printf -v _TERMINAL_C1_CTRL[$_code] '%b' "\\302\\${_octal}"
  printf -v _TERMINAL_C1_REPL[$_code] '\\u%04X' "$_code"
done
unset _code _octal
...
terminal_safe_text() {
  ...
  for code in {1..31}; do
    value=${value//"${_TERMINAL_C0_CTRL[$code]}"/"${_TERMINAL_C0_REPL[$code]}"}
  done
  value=${value//$'\177'/\\x7F}
  ...
  for code in {128..159}; do
    value=${value//"${_TERMINAL_C1_CTRL[$code]}"/"${_TERMINAL_C1_REPL[$code]}"}
  done
