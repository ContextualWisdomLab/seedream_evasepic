...
Implementation evidence: `3694ef7b3f8ed7a6c7a8c56c7986b824f472d0b0`; zero-byte contract repair: `8f77158774607e5f14ef4d99c2e42c30f6383d82`.

The download writes to a private `0700` staging directory created beside the requested output. After `yt-dlp` completes, a regular staged artifact is renamed onto the final path on the same filesystem. A late final-component symlink is therefore replaced as a directory entry instead of becoming `yt-dlp`'s write target. Existing cache-hit and zero-byte cache-miss semantics are preserved; a successful `yt-dlp` invocation that produces a zero-byte regular artifact is not reclassified by this security repair.

The `.jules/sentinel.md` file remains aligned with the protected-base evidence rather than generalizing this bounded repair into a universal symlink-TOCTOU rule.
