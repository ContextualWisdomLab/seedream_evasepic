...
and then selects the largest IEC 80000-13 binary prefix that the count reaches:
...
`awk` dependency on the cache-hit test PATH, which must not contain `yt-dlp`.
The formatter rejects a non-numeric count instead of letting `[ -ge ]` abort
with an opaque comparison error.
...
- the existing 26-byte cache-hit payload must print `Size: 26 bytes`;
...
- a mocked `yt-dlp` that writes exactly 2,048 KiB must print `Size: 2.00 MiB`
  and must not print `Size: 2.00 MB`; and
...
National Institute of Standards and Technology. (2008). *Guide for the use of
the International System of Units (SI)* (NIST Special Publication 811, 2008
ed.). https://doi.org/10.6028/NIST.SP.811e2008
