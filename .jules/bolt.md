...
## 2025-02-19 - [Bash 성능 개선] 비싼 네트워크 작업의 조기 반환(Early Return) 캐싱
**Learning:** `yt-dlp`와 같은 외부 도구는 파일이 이미 존재하더라도 네트워크 요청을 보내 메타데이터를 확인하는 과정에서 수 초(1~3초)의 지연이 발생할 수 있다. 동일한 파일을 다시 다운로드할 때 이 오버헤드가 누적된다.
**Action:** 비싼 네트워크 I/O가 발생하는 다운로드 스크립트에서는, 실행 전 대상 파일이 존재하는지(`[ -s "$OUTPUT" ]`)를 검사하여 이미 다운로드 된 경우 조기 반환(early return)하도록 처리해 불필요한 네트워크 통신 오버헤드를 원천적으로 차단한다.

## 2025-02-19 - [Bash 성능 개선] 비싼 네트워크 작업 조기 반환 시 디렉토리 예외 처리
**Learning:** `[ -s "$OUTPUT" ]`와 같은 파일 크기 체크는 대상이 디렉토리일 경우에도 `true`를 반환하기 때문에 예기치 않은 오류나 잘못된 조기 반환을 발생시킬 수 있다.
**Action:** 파일의 존재 여부와 크기를 기반으로 조기 반환(early return)을 구현할 때는 반드시 `[ -f "$OUTPUT" ] && [ -s "$OUTPUT" ]`와 같이 정규 파일 여부를 함께 검사하여 오탐을 방지한다.
