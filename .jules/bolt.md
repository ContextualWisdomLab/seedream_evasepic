...
## 2026-09-14 - Bash Performance Overhead from Built-ins in Loops
**Learning:** Repetitive string manipulations across a known set of characters (like C0 controls) using a `for` loop with `printf -v` inside Bash computes parameter expansions iteratively per item, which causes execution overhead due to repeated built-in invocations.
