...
## 2024-08-29 - [Eliminate Subshell Overhead in Bash Utility Functions]
**Learning:** In Bash, capturing utility function output using subshells (e.g., `result="$(my_func)"`) introduces significant performance overhead due to process forking, scaling linearly with the number of calls. This is a common performance bottleneck in shell scripts.
**Action:** Avoid subshells in utility functions by allowing them to accept an optional output variable name as an argument. Use `printf -v <var>` to assign the result directly to that variable if provided, while falling back to standard output (e.g., `printf`) if the argument is omitted, preserving API backward compatibility. Ensure to explicitly initialize the output variable (e.g., `safe_value=""`) in the caller before using `printf -v` to prevent static analysis errors (SC2154).
