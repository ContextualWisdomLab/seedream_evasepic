...
## 2024-05-24 - Bash string replacement performance
**Learning:** In Bash, performing string substitutions using direct, hardcoded parameter expansions (e.g., `${var//pattern/replacement}`) is significantly faster than using loops with built-in commands like `printf -v`. Also capturing function output via subshells (e.g., `result="$(my_func)"`) introduces significant performance overhead due to process forking.
**Action:** Replace loops generating parameter expansions dynamically with direct, unrolled, hardcoded parameter expansions. Accept an optional output variable name as an argument and use `printf -v` to assign the result directly to that variable to avoid subshells in utility functions.
