...
## 2024-05-18 - [CRITICAL] 루프 내 printf 호출을 배열 사전 계산으로 대체
**Learning:** Bash에서 변수 처리를 위해 루프 안에서 `printf -v` 같은 내장 명령어를 반복 호출하는 것은 심각한 성능 저하를 초래합니다.
**Action:** 스크립트 로드 시 전역 배열에 필요한 값을 한 번만 미리 계산해두고, 함수 내에서는 배열을 참조하여 치환하는 방식으로 최적화하여 런타임 오버헤드를 줄이십시오.
