...
## 2024-07-13 - [Bash 성능 개선] tr 명령어 대체로 파이프 및 서브셸 오버헤드 제거
**Learning:** `tr`과 같은 외부 명령어를 문자열 조작을 위해 파이프라인으로 연결하면 서브셸 생성 및 프로세스 포크 오버헤드가 발생하여 성능이 저하됩니다.
**Action:** 쉘 스크립트에서 단순 공백 제거나 문자열 치환 작업 시 외부 도구(`tr`, `sed` 등) 대신 Bash 내장 파라미터 확장(Parameter Expansion, 예: `"${VAR//[[:space:]]/}"`)을 사용하여 프로세스 생성 비용을 없애고 성능을 향상시킵니다.
