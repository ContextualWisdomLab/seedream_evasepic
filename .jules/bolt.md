...
## 2024-05-20 - Bash 반복문 최적화 (printf -v 회피)
**Learning:** Bash 내장 명령인 `printf -v`를 반복문(특히 자주 호출되는 함수 안)에서 반복 실행하면 성능 저하가 큽니다.
**Action:** 루프 내부에서 동적으로 생성되던 고정된 값(제어 문자 대체 문자열 등)을 스크립트 로드 시점에 전역 배열로 미리 계산(Precompute)하여 캐싱함으로써 함수 실행 속도를 크게 개선할 수 있습니다. 오래된 Bash 버전을 지원하기 위해 `printf -v`의 결과를 직접 배열 요소에 할당하지 말고 임시 스칼라 변수를 거쳐 할당해야 합니다.
