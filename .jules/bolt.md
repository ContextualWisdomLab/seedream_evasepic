...
## 2026-09-24 - [Bash 성능 개선] Subshell 생성 오버헤드 제거
**Learning:** 쉘 스크립트의 함수 내에서 결괏값을 가져오기 위해 \`$()\` (Command Substitution) 서브셸을 생성하면, 서브셸 생성 및 파이프라이닝 오버헤드로 인해 성능이 눈에 띄게 저하됩니다.
**Action:** 결괏값을 문자열로 반환하는 대신, \`printf -v\` 와 같은 기능을 활용하여 호출 측의 변수 이름(nameref)으로 직접 값을 할당하도록 변경하면 서브셸 생성 없이 성능을 최적화할 수 있습니다.
