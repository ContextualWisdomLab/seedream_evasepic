...
## 2024-05-24 - Bash Loop Unrolling for Parameter Expansion
**Learning:** Built-in command invocations (like `printf`) inside Bash loops carry significant execution overhead. For repetitive string replacements across a known set of characters, unrolling the loop into direct parameter expansions (e.g., `value=${value//pattern/replacement}`) is vastly faster.
**Action:** When performing deterministic character neutralizations or replacements in Bash, avoid iterating with `for` loops and `printf`; use statically generated, unrolled parameter expansions to minimize subshell/invocation overhead.
