...
## 2024-07-26 - [Bash 성능 개선] 외부 명령어(dirname) 호출 오버헤드 방지
**Learning:** 스크립트 내에서 `dirname`과 같은 외부 명령어를 서브셸로 호출하면 프로세스 포크 오버헤드가 발생하여 성능이 저하됩니다.
**Action:** 외부 도구(dirname) 대신 Bash 내장 파라미터 확장(예: `"${VAR%/*}"`)을 사용하여 순수 Bash 내장 기능만으로 문자열 조작을 수행해 프로세스 생성 비용을 없애고 성능을 향상시킵니다.
