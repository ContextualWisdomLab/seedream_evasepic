...
## 2024-08-14 - Bash 파라미터 확장을 이용한 루프 성능 최적화
**Learning:** Bash에서 문자열 치환을 위해 `for` 루프와 `printf -v`를 사용하면 프로세스 호출과 유사한 오버헤드로 인해 심각한 병목 현상이 발생할 수 있습니다.
**Action:** `for` 루프 대신 직접 하드코딩된 파라미터 확장(`value=${value//pattern/replacement}`)을 사용하면 오버헤드를 줄여 문자열 치환 성능을 크게 향상시킬 수 있습니다.
