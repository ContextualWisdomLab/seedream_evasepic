...
## 2025-06-25 - [Bash 성능 개선] yt-dlp 중복 다운로드 방지 조기 반환
**Learning:** 네트워크 통신과 처리 시간이 오래 걸리는 `yt-dlp` 같은 외부 CLI 도구를 매번 호출하면 심각한 성능 저하가 발생합니다. 하지만 `-s` 옵션은 디렉터리에 대해서도 참을 반환할 수 있으므로 주의해야 합니다.
**Action:** 네트워크 다운로드나 처리 작업을 수행하기 전에 반드시 `[ -f "$OUTPUT" ] && [ -s "$OUTPUT" ]` 체크를 통해 대상 파일이 이미 존재하고 크기가 0보다 큰지 확인하여 불필요한 실행을 건너뜁니다.
