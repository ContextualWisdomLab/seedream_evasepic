...
## 2024-09-13 - [Loop unrolling in Bash]
**Learning:** Using `for` loops in Bash for character replacement via parameter expansion (e.g. `printf -v` inside the loop and `${value//search/replace}`) causes significant overhead when dealing with multiple control characters.
**Action:** Unroll loops that perform static string replacements on a known set of characters into direct parameter expansions (e.g., `value=${value//$'\001'/\x01}`) to avoid execution overhead.
