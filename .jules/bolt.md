...
## 2025-06-25 - [Bash 성능 개선] 무거운 네트워크 도구 호출 전 Early Return
**Learning:** `yt-dlp`와 같은 외부 무거운 프로세스나 네트워크 요청을 수행하는 CLI 도구는 호출 자체로도 오버헤드가 크며 불필요한 네트워크 대역폭을 낭비할 수 있습니다.
**Action:** 대상 출력 파일이 이미 존재하고 비어있지 않은지(`[ -f "$OUTPUT" ] && [ -s "$OUTPUT" ]`) 사전에 확인하여, 조기 종료(Early Return) 처리함으로써 불필요한 프로세스 생성 및 중복 작업을 방지합니다. 단순 `-s` 플래그는 디렉터리에도 참(true)을 반환할 수 있으므로 항상 `-f`와 함께 사용해야 합니다.
