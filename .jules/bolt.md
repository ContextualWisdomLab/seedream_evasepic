...
## 2024-09-07 - Bash Scripting (Performance)
**Learning:** Dynamically generating string replacement rules with `printf` inside a loop in bash (e.g., `printf -v` inside a `for` loop) carries significant execution overhead, especially for core string-manipulation utilities called frequently.
**Action:** Unroll the loop into direct parameter expansions (e.g., `value=${value//$'\001'/\x01}`) to avoid loop overhead and built-in command invocations, achieving a significant performance improvement (in this case, ~4x faster execution time for string neutralization).
