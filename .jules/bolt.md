...
## 2024-03-22 - Bash printf -v 반복 호출 오버헤드 개선
**Learning:** Bash 쉘 스크립트에서 문자열 치환을 위해 루프 내에서 빌트인 명령어인 `printf -v`를 반복적으로 호출하는 것은 상당한 성능 저하를 일으킵니다. 특히 입력 문자열의 크기가 크거나 함수가 빈번하게 호출될 때 오버헤드가 누적됩니다.
**Action:** 루프 내부에서 동적으로 이스케이프 시퀀스를 생성하는 대신, 스크립트 로드 시 전역 배열(global arrays)에 한 번만 미리 계산하여 캐싱(precompute)하는 방식을 사용합니다. 이후 루프에서는 캐싱된 배열의 값을 참조하여 성능을 크게 향상시킬 수 있습니다.
