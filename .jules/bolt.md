...
## 2024-05-24 - Bash 문자열 치환 성능 최적화 (루프 언롤링)
**Learning:** Bash 스크립트 내부에서 텍스트 처리 시 `printf -v`와 같은 내장 명령어를 반복문(loop) 안에서 지속적으로 호출하면 상당한 성능 저하(오버헤드)가 발생합니다. 동적 스코프 하에서 변수를 할당하고 연산하는 과정이 느리기 때문입니다.
**Action:** 텍스트 처리와 같이 빈번하게 호출되는 유틸리티 함수에서는, 동적 변수 할당을 피하고 가능한 정적인 하드코딩 치환(예: `${value//pattern/replacement}`)으로 루프를 언롤링(unroll)하여 성능을 개선합니다.
