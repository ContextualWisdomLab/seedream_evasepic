...
## $(date +%Y-%m-%d) - [Bash 성능 개선] 서브셸 및 내부 루프 오버헤드 최적화
**Learning:** `printf`와 서브셸이 포함된 루프를 통해 동적으로 치환 패턴을 생성하는 것은 비용이 매우 비쌉니다.
**Action:** `for` 루프 대신 직접 하드코딩된 Bash 네이티브 파라미터 확장을 사용(예: `value=${value//$'\001'/\x01}`)하면, 서브셸 생성 비용과 문자열 생성 비용을 제거하여 성능을 비약적으로 끌어올릴 수 있습니다.
