...
## 2024-08-30 - Bash terminal neutralization loop overhead
**Learning:** In bash scripts processing strings, repeatedly invoking builtins (like `printf -v`) inside loops for static text replacements (e.g. neutralizing control characters) introduces significant overhead. Unrolling these loops into direct sequential parameter expansions maximizes performance and significantly cuts down time.
**Action:** Unroll loops that perform static string replacements via parameter expansion inside bash scripts, especially in utility functions that could be called frequently.
