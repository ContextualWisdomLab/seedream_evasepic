...
## 2024-07-29 - [yt-dlp 다운로드 최적화] DASH/HLS 스트림 병렬 다운로드
**Learning:** yt-dlp는 기본적으로 비디오 프래그먼트를 순차적으로 다운로드하기 때문에, 특히 DASH/HLS와 같은 스트리밍 포맷을 받을 때 네트워크 병목 현상이 발생하여 속도가 느려집니다.
**Action:** yt-dlp 호출 시 `--concurrent-fragments N` 옵션(예: 4)을 추가하여 여러 프래그먼트를 병렬로 다운로드하게 함으로써 I/O 대기 시간을 줄이고 전체 다운로드 속도를 크게 향상시킵니다.
