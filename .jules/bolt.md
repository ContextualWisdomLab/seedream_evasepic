...
## 2025-02-19 - [Bash 성능 개선] 터미널 출력 이스케이프 문자열 조작 시 서브셸 및 루프 최소화
**Learning:** `terminal_safe_text` 함수 내에서 C0 및 C1 제어 문자 처리를 위해 `for` 루프와 `printf -v`를 사용하면 내부적인 반복과 서브셸 오버헤드가 발생하여 처리 시간이 크게 증가합니다. 또한 함수 결과를 캡처하기 위해 서브셸(`$()`)을 사용하면 프로세스 포크 비용이 발생합니다.
**Action:** 단순한 문자열 매핑 작업의 경우 `for` 루프 대신 Bash 네이티브 파라미터 확장(예: `${value//$'\001'/\x01}`)을 하드코딩하여 성능을 향상시킵니다. 또한 유틸리티 함수에서 서브셸 캡처를 피하기 위해, 반환값을 저장할 변수명을 인자로 전달받아 `printf -v`로 직접 할당하도록 수정합니다.
