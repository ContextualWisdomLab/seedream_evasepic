...
## 2024-08-25 - [Bash 성능 개선] Subshell 없는 함수 반환 (printf -v)
**Learning:** Bash에서 함수 실행 결과를 `$(func)` 형식의 서브셸(subshell)로 캡처하면 매번 프로세스 fork(복제) 비용이 발생하여 스크립트 실행이 상당히 느려질 수 있습니다. 특히 루프 내에서 많이 호출되는 함수에서 이러한 현상이 두드러집니다.
**Action:** 함수 설계 시 출력값을 담을 변수 이름을 선택적 매개변수(optional argument)로 전달받도록 하고, 해당 인자가 있을 경우 `printf -v "$__out_var"` 등을 통해 서브셸 없이 직접 변수에 값을 할당하는 방식으로 구현하여 성능을 개선합니다.
