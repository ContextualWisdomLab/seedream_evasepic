...
## 2025-10-25 - [Bash 성능 개선] 불필요한 네트워크 다운로드 조기 종료 처리
**Learning:** 대상 파일이 이미 존재함에도 불구하고 무거운 CLI 네트워크 도구(`yt-dlp` 등)를 호출하면 불필요한 네트워크 요청 및 프로세스 오버헤드가 발생합니다. `-s` 플래그는 디렉터리에도 참(true)을 반환하므로 주의해야 합니다.
**Action:** 무거운 처리 전에 항상 대상 파일의 존재 여부를 확인하여 조기 반환(early return)을 구현합니다. 디렉터리로 인한 오작동(false-positive)을 방지하기 위해 파일 여부를 먼저 검증하는 `[ -f "$OUTPUT" ] && [ -s "$OUTPUT" ]` 패턴을 사용합니다.
