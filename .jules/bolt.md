...
## 2026-07-28 - [Bash 성능 개선] 서브셸 캡처 오버헤드 방지
**Learning:** 쉘 스크립트에서 함수의 출력을 `$(...)` 서브셸로 캡처하여 변수에 할당하는 것은 프로세스 포크 오버헤드를 유발하여 성능을 눈에 띄게 저하시킵니다.
**Action:** 유틸리티 함수가 결과값을 반환해야 할 때, 선택적 출력 변수 이름을 인자로 받아 `printf -v`를 통해 변수에 직접 결과를 할당하도록 변경하면 서브셸 생성을 피하면서 하위 호환성도 유지할 수 있습니다.
