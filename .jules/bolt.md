...
## 2025-06-26 - [Bash 성능 개선] 정적 문자열 치환 루프 전개 최적화
**Learning:** 셸 스크립트 내에서 외부 명령어 호출이나 복잡한 포맷팅 연산(예: 루프 내 반복적인 printf 호출)을 사용하면 상당한 성능 저하가 발생합니다. 특히 매우 짧은 함수나 빈번하게 호출되는 곳에서는 이러한 오버헤드가 누적됩니다.
**Action:** 루프에서 정적이고 고정된 횟수(예: 제어 문자 처리)로 반복되는 작업은 직접 변수 치환 구문을 순차적으로 전개(unroll)하여 작성하면 프로세스 포크와 커맨드 실행 비용을 완전히 없애고 극적인 성능 향상을 달성할 수 있습니다.
