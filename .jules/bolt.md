...
## 2025-06-25 - [Bash 성능 개선] 하드코딩된 파라미터 확장으로 문자열 치환 루프 최적화
**Learning:** 문자열 치환을 위해 `printf -v`로 패턴과 교체 문자열을 동적으로 생성하여 `for` 루프 안에서 치환하면 내부 명령 평가 비용이 크게 발생하여 성능이 저하된다. Bash의 하드코딩된 파라미터 확장(`${var//pattern/replacement}`)을 직접 명시하는 것이 훨씬 빠르다.
**Action:** 미리 정해진 제어 문자들의 집합을 치환할 때는 루프 대신 직접 확장된 파라미터 교체 방식을 여러 줄에 걸쳐 나열함으로써 성능을 최적화한다.
