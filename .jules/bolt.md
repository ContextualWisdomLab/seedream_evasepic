...
## 2026-07-22 - Avoid Redundant Execution for Network/Heavy Processing Scripts
**Learning:** Heavy CLI tools (like `yt-dlp`) impose significant time cost and unnecessary network usage if run repeatedly. Using just `-s` to check if an output exists can cause false-positive skips if the path is a directory.
**Action:** Always implement early return checks using `[ -f "$OUTPUT" ] && [ -s "$OUTPUT" ]` before invoking heavy external network or processing CLI tools to avoid expensive redundant execution.
