...
## 2024-08-24 - [Bash 성능 개선] 반복문과 외부 명령어를 활용한 문자열 조작 오버헤드 방지
**Learning:** 셸 스크립트에서 터미널 제어 문자 필터링 등을 위해 `for` 루프 안에서 `printf`를 반복적으로 호출하면 매 반복마다 서브셸이나 내장 명령어 처리에 오버헤드가 발생하여 성능이 크게 저하됩니다. (1000번 반복 테스트 시 1.3초 vs 0.38초)
**Action:** `for` 루프 내부에서 `printf` 등을 사용하는 대신, 필요한 변환 과정을 Bash 내장 파라미터 확장(`value=${value//pattern/replacement}`)을 이용해 명시적으로 루프 밖으로 펼쳐서(unroll) 작성함으로써 프로세스/명령어 호출 비용을 제거하고 성능을 극대화합니다.
