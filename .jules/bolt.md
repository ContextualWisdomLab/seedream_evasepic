...
## 2026-07-28 - [Bash 성능 개선] printf 서브셸 및 반복 호출 회피
**Learning:** `terminal_safe_text` 함수 내에서 제어 문자를 중화하기 위해 `for` 루프 내에서 매번 `printf`를 반복 호출하는 것은 프로세스 생성 오버헤드와 루프 반복 오버헤드를 유발하여 상당한 성능 저하를 일으킵니다.
**Action:** `terminal_safe_text`와 같이 반복적으로 호출되는 문자열 처리 함수에서는 `printf`를 통한 문자열 조작을 런타임에 반복하지 않고, 배열을 사용해 사전에 필요한 대체 문자열을 한 번만 계산(pre-compute)하여 캐싱한 뒤 런타임에는 캐싱된 배열을 사용해 대체하도록 최적화합니다.
