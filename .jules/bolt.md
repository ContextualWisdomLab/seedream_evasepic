...
## 2024-05-24 - yt-dlp 병렬 다운로드 최적화
**Learning:** yt-dlp는 기본적으로 DASH/HLS 스트림을 단일 스레드로 다운로드하여 네트워크 대역폭을 충분히 활용하지 못하는 병목 현상이 발생함.
**Action:** `yt-dlp` 호출 시 `--concurrent-fragments N` (예: 4) 옵션을 추가하여 프래그먼트 병렬 다운로드를 활성화함으로써 다운로드 속도를 크게 향상시킬 수 있음.
