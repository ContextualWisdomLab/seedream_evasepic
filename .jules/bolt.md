...
## 2024-05-18 - 터미널 출력 제어 문자 중화 성능 최적화
**Learning:** Bash 스크립트에서 `printf -v`와 같은 내장 명령어를 반복문 내부에서 자주 호출하면 심각한 성능 저하가 발생합니다. 특히 터미널 출력을 위해 제어 문자를 이스케이프하는 과정에서 매번 63번의 `printf -v` 호출은 큰 병목이 됩니다.
**Action:** 전역 배열을 사용하여 스크립트 로드 시점에 제어 문자와 대체 문자열을 미리 계산하고 저장함으로써, 함수 호출 시 `printf -v` 호출을 피하고 성능을 향상시켰습니다.
## 2024-05-18 - 터미널 출력 제어 문자 중화 성능 최적화 개선
**Learning:** Bash 스크립트에서 글로벌 배열을 정의할 때 `declare -a`를 사용하면, 만약 이 스크립트가 함수 내부에서 `source` 될 경우(예: 테스트 프레임워크의 setup 함수 등) 해당 배열이 함수 스코프로 갇히게 됩니다. 이로 인해 스크립트의 나머지 부분이나 외부 함수 호출 시 배열을 참조할 수 없어 치명적인 버그가 발생합니다.
**Action:** `declare -g -a`를 사용하여 함수 내부에서 `source` 되더라도 배열이 전역(global)으로 선언되도록 수정하였습니다. 이를 통해 스코프 문제를 해결하고 안정성을 보장했습니다. 또한 100% 테스트 커버리지를 위해 테스트 코드를 추가했습니다.
## 2024-05-18 - 터미널 출력 제어 문자 중화 성능 최적화 글로벌 배열 선언 수정
**Learning:** `declare -g -a`를 사용하면 Bash 4.2 이상에서만 동작하므로, Bash 3.2를 사용하는 macOS 등 특정 환경이나 구형 CI에서는 "invalid option" 오류와 함께 스크립트가 크래시됩니다.
**Action:** `declare` 키워드를 아예 제거하였습니다. Bash에서 할당문을 통해 배열을 초기화하면, 동일한 이름의 로컬 변수가 존재하지 않는 한 함수 내에서 실행되더라도 전역 배열로 자동 생성됩니다. 이를 통해 Bash 3.2 호환성과 전역 스코프 접근성 두 가지를 모두 만족하도록 수정했습니다.
