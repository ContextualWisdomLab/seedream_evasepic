...
## 2024-07-13 - [네트워크 I/O 최적화] yt-dlp 동시 다운로드 적용
**Learning:** DASH/HLS 스트림을 다운로드할 때 기본 설정으로 `yt-dlp`를 실행하면 단일 연결로 인해 네트워크 병목이 발생하여 속도가 느려질 수 있습니다.
**Action:** 앞으로 `yt-dlp`를 스크립트에 통합할 때는 `--concurrent-fragments N` 플래그(예: `--concurrent-fragments 4`)를 추가하여 세그먼트를 병렬로 가져옴으로써 다운로드 속도를 크게 향상시키고 네트워크 I/O 병목을 줄여야 합니다.
