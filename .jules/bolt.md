...
## 2025-02-19 - Bash 성능 개선: 제어 문자 이스케이프 루프 언롤링
**Learning:** Bash에서 C0/C1 제어 문자를 이스케이프할 때 `for` 루프 내부에서 `printf -v`를 반복 호출하면 루프 및 내장 명령어 호출 오버헤드가 크게 발생합니다. 특히 문자의 집합이 고정되어 있는 경우에는 루프를 사용하는 대신 직접 파라미터 확장을 나열(unrolling)하는 것이 성능 향상에 도움이 됩니다.
**Action:** 앞으로 알려진 고정된 문자 집합(예: C0/C1 제어 문자)을 다룰 때는 `for` 루프와 `printf -v` 조합 대신 직접 문자열 치환(`${var//pattern/replacement}`)을 명시적으로 나열하여 루프 오버헤드를 제거합니다.
