...
## 2024-09-09 - [Bash Loop Unrolling Optimization]
**Learning:** Using `printf -v` repeatedly inside a bash loop introduces significant performance overhead for text processing functions.
**Action:** Unroll loops into static, hardcoded parameter expansions (e.g., `value=${value//pattern/replacement}`) to avoid runtime overhead in text sanitization loops.
