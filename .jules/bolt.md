...
## 2024-07-28 - [Bash 성능 개선] 반복문 언롤링을 통한 이스케이핑 속도 최적화
**Learning:** 쉘 스크립트에서 단일 문자열 내의 여러 문자를 이스케이핑할 때, `for` 루프와 `printf` 등의 내장 명령어를 반복 호출하면 서브쉘과 유사한 프로세스 포크 오버헤드가 누적되어 처리 성능이 크게 저하됩니다.
**Action:** 고정된 패턴 변환이 다수 필요한 경우, 반복문 대신 매직 넘버와 `value=${value//pattern/replacement}` 형식의 직접 파라미터 확장을 사용하여 반복문을 언롤링(unroll)함으로써 스크립트의 성능을 극대화합니다.
