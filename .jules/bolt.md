...
## 2025-02-23 - [Bash 성능 개선] 반복문 내 printf 포맷팅 오버헤드 최적화
**Learning:** 쉘 스크립트에서 `for` 반복문 내부에서 `printf -v`를 사용해 동적으로 제어 문자를 이스케이프하는 작업은 서브셸은 아니지만 루프 오버헤드가 발생해 성능에 부정적인 영향을 미칩니다 (ex: 약 3배 지연).
**Action:** `terminal_safe_text`와 같이 자주 호출되는 함수에서 정적 문자를 처리할 때는 반복문을 하드코딩된 파라미터 확장(예: `value=${value//$'\001'/\\x01}`)으로 풀어(Unroll) 루프 오버헤드를 제거합니다.
