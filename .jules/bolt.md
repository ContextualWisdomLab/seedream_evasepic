...
## 2026-07-21 - [Bash 성능 개선] 불필요한 네트워크/외부 도구 실행 방지 (Early Return)
**Learning:** 대상 파일이 이미 존재함에도 불구하고 무조건적으로 yt-dlp 등의 무거운 외부 네트워크 도구를 실행하면 불필요한 프로세스 오버헤드와 네트워크 대기 시간이 발생합니다.
**Action:** 항상 무거운 외부 도구를 실행하거나 네트워크 요청을 보내기 전에 목표 결과물(파일 등)이 이미 존재하는지(`[ -f "$OUTPUT" ] && [ -s "$OUTPUT" ]`) 검사하는 early return 구문을 추가하여 실행을 최적화합니다. 특히 단순 `-s`는 디렉토리에도 참이 될 수 있으므로 `-f`와 함께 검사해야 합니다.
