...
## 2025-09-06 - [Bash 성능 개선] 터미널 출력 이스케이프 루프 전개(Loop Unrolling)
**Learning:** Bash 쉘 스크립트에서 터미널 출력용 텍스트를 이스케이프 처리할 때, `printf`와 서브쉘, 또는 동적 변수 할당을 수십 번 반복하는 루프(`for code in {1..31}; do ... printf -v ... done`)는 엄청난 프로세스/서브쉘 생성 및 Bash 자체 루프 오버헤드를 유발하여 문자열 처리 성능을 심각하게 저하시킵니다.
**Action:** 쉘 스크립트에서 고정된 제어 문자(C0, C1 등) 집합을 치환할 때는, 런타임에 계산하는 루프를 사용하는 대신 미리 계산된 패턴 교체(예: `value=${value//$''/\x01}`)를 정적으로 풀어서(Loop Unroll) 작성합니다. 이렇게 하면 런타임 루프 및 동적 변수 할당 비용 없이 단일 Bash 매개변수 확장의 내부 처리만으로 성능이 비약적으로 향상됩니다.
