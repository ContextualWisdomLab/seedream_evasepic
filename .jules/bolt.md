...
## 2024-07-26 - [Bash Performance] Unroll loops for string manipulation
**Learning:** Using `for` loops with `printf -v` inside Bash functions to dynamically generate search and replace patterns for string manipulation (like neutralizing control characters) carries significant execution overhead.
**Action:** When performing repetitive string manipulations across a known set of characters, unroll the loop into direct parameter expansions (e.g., `value=${value//$'\001'/\x01}`) to eliminate loop overhead and improve script execution speed.
