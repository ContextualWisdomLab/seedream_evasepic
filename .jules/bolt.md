...
## 2026-08-31 - [Bash 성능 개선] 터미널 텍스트 이스케이프 루프 언롤링 최적화
**Learning:** for 루프 내부에서 printf -v를 반복 호출하여 문자 치환을 수행하면 상당한 서브셸과 서식 지정 오버헤드가 발생합니다. 특히 C0/C1 제어 문자(31+32개)처럼 고정된 크기일 때는, 반복문 대신 변수 치환(Parameter expansion)을 언롤링하여 하드코딩하면 수행 시간이 크게 향상됩니다.
**Action:** 쉘 스크립트에서 작은 고정 횟수의 루프와 내부 함수 호출(printf 등)이 반복되는 경우, 코드 라인 수가 다소 늘어나더라도 반복문을 하드코딩된 파라미터 확장으로 언롤링(Unrolling)하여 성능을 최적화합니다.
