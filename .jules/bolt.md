...
## 2024-05-18 - Bash Loop Unrolling Performance
**Learning:** Using `printf -v` inside a `for` loop in Bash has significant execution overhead due to the repeated invocation of the built-in command. Unrolling these loops into direct parameter expansions (e.g., `value=${value//$'\001'/\x01}`) provides substantial performance benefits (roughly 3x faster).
**Action:** When performing repetitive string manipulations across a known set of characters or values in Bash, unroll the loop into direct parameter expansions instead of using a `for` loop with `printf -v`.
