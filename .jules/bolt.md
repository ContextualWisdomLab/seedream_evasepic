...
**Learning:** 스크립트 내에서 단순한 문자열 조작(공백 제거 등)을 위해 파이프라인과 외부 명령어(`tr`)를 호출하면, 불필요한 서브셸과 프로세스가 생성되어 성능 저하가 발생합니다.
**Action:** 외부 명령어(`tr`) 대신 Bash 내장 파라미터 확장(예: `"${VAR//[[:space:]]/}"`)을 활용하여 문자열을 처리함으로써 프로세스 생성 비용을 없애고 성능을 향상시킵니다.
