...
## 2025-10-23 - [Bash 성능 개선] 유틸리티 함수에서 불필요한 서브셸 방지
**Learning:** Bash에서 함수 출력을 캡처하기 위해 서브셸(예: `result="$(my_func)"`)을 사용하면 프로세스 포크로 인해 상당한 성능 오버헤드가 발생합니다.
**Action:** API의 하위 호환성을 깨지 않고 서브셸 오버헤드를 피하려면, 선택적 출력 변수 이름을 인수로 받아 `printf -v`를 사용해 결과를 해당 변수에 직접 할당하고, 인수가 생략된 경우에만 표준 출력(예: `printf`)으로 폴백하도록 함수를 설계합니다.
