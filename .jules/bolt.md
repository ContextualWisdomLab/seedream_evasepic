...
## 2024-09-18 - [Bash 배열 사전 연산을 통한 성능 최적화]
**Learning:** Bash 루프 내에서 `printf -v` 같은 빌트인 명령어를 반복적으로 실행하면 상당한 성능 오버헤드가 발생합니다. 반복문을 완전히 풀어서(unrolling) 전개하면 코드가 길어지고 가독성이 떨어지며, 50라인 미만의 최적화 제약에도 위배될 수 있습니다.
**Action:** 스크립트 로드 시(load time) 전역 배열에 필요한 값들을 미리 계산해(precompute) 저장하고, 실제 함수 호출 시에는 배열의 인덱스를 통해 참조하도록 수정하여 가독성을 해치지 않으면서도 함수 실행 속도를 크게 개선합니다.
