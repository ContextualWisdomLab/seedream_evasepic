...
## 2024-08-07 - [Bash 성능 개선] 외부 명령어(tr) 호출 파이프 오버헤드 방지
**Learning:** 파일 크기 문자열의 공백 제거 단계에서 파이프라인과 외부 `tr`를 사용하면, 이미 필요한 `wc -c` 명령 치환에 더해 추가 프로세스와 파이프라인 오버헤드가 발생합니다.
**Action:** 이 공백 제거 단계에는 Bash 내장 파라미터 확장(예: `"${VAR//[[:space:]]/}"`)을 사용해 추가 `tr` 프로세스와 파이프라인을 제거합니다. 파일 크기 측정을 위한 `$(wc -c < "$OUTPUT")` 명령 치환 자체는 그대로 남아 있으므로, 모든 서브프로세스 비용이 제거된 것으로 간주하지 않습니다.
