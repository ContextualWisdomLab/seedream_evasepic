...
## 2024-09-13 - [Bash 성능 개선] 루프 내 printf 호출 오버헤드 방지
**Learning:** 쉘 스크립트에서 문자열 제어 문자 등을 치환할 때 루프 내에서 내장 명령어인 `printf -v`를 반복적으로 호출하면 동적 변수 할당과 파싱으로 인해 상당한 성능 오버헤드가 발생합니다.
**Action:** 성능이 중요한 문자열 처리 함수에서는 루프를 사용하는 대신, 정적으로 풀어진(unrolled) Bash 내장 파라미터 확장(예: `value=${value//$'\001'/\\x01}`)을 하드코딩하여 프로세싱 오버헤드를 획기적으로 줄여야 합니다.
