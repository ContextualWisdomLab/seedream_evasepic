...
## 2024-05-18 - [Bash 성능 개선] 무거운 외부 프로세스(yt-dlp) 호출 전 조기 종료(Early Return) 도입
**Learning:** 파일 다운로드나 외부 네트워크 통신 등 무겁고 시간이 오래 걸리는 작업(yt-dlp 등)을 수행하기 전에 결과 파일이 이미 존재하는지 확인하지 않으면 불필요한 프로세스 실행 및 네트워크 지연 오버헤드가 발생합니다. 이때 단순히 `-s` 옵션만 사용하면 디렉터리를 파일이 존재하는 것으로 잘못 판단할 수 있습니다.
**Action:** 네트워크 요청이나 무거운 처리(yt-dlp 등)를 수행하는 쉘 스크립트에서는 작업 전 반드시 `[ -f "$OUTPUT" ] && [ -s "$OUTPUT" ]` 조건문으로 조기 종료(Early return)를 구현하여 중복 실행 비용을 절약합니다.
