...
## 2026-08-04 - [Bash 성능 개선] tr 등 외부 프로세스를 파라미터 확장으로 교체
**Learning:** 문자열 조작(예: 공백 제거) 시 외부 유틸리티인 `tr`을 서브셸로 호출하면 fork/exec 오버헤드가 발생합니다.
**Action:** 단순 문자열 공백 제거 시, 외부 도구 대신 Bash의 내장 파라미터 확장 `${VAR//[[:space:]]/}` 등을 사용하여 서브셸 생성 및 프로세스 호출 비용을 없애 최적화합니다.
