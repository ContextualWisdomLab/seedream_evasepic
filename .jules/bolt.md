...
## 2024-08-11 - [Bash 성능 개선] 터미널 출력 문자 중화 루프 오버헤드 방지
**Learning:** 터미널 출력 시 제어 문자를 이스케이프하기 위해 `printf -v`와 루프를 사용하면 호출 빈도가 높을 경우 성능 저하가 발생합니다.
**Action:** 쉘 스크립트에서 고정된 문자의 변환이 빈번히 발생할 경우, 루프와 내장 명령어를 사용하는 대신 Bash의 파라미터 확장 기능(예: `value="${value//$'\001'/\\x01}"`)을 통해 루프를 언롤링(Unrolling)하여 서브쉘 및 프로세스 오버헤드 없이 성능을 최적화합니다.
