...
## 2024-05-24 - Bash 문자열 치환 최적화 (루프 전개)
**Learning:** Bash 스크립트에서 루프 안에서 `printf -v`를 사용해 문자열을 반복적으로 치환하는 것은 심각한 성능 저하를 일으킵니다. `value=${value//$'\001'/\\x01}`와 같이 하드코딩된 정적 확장을 사용하여 루프를 전개(unrolling)하면 실행 속도를 3~4배 이상 향상시킬 수 있습니다.
**Action:** Bash 스크립트에서 대량의 문자열 치환이나 이스케이프 처리가 필요할 때는 성능 향상을 위해 루프 대신 전개(unrolling)된 방식을 사용하십시오.
