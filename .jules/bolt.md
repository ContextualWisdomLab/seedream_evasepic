...
## 2025-07-26 - [Bash 성능 개선] yt-dlp 중복 다운로드 방지 조기 반환 (Early Return)
**Learning:** 다운로드 스크립트에서 파일이 이미 존재하는지 확인하지 않고 무조건 `yt-dlp`를 호출하면 불필요한 네트워크 요청과 프로세스 생성 오버헤드가 크게 발생합니다.
**Action:** 다운로드와 같이 비용이 큰 작업을 수행하기 전에, `[ -f "$OUTPUT" ] && [ -s "$OUTPUT" ]`와 같이 대상 파일이 이미 존재하고 크기가 0이 아닌지 확인하여(Early Return) 중복 실행과 오버헤드를 사전에 차단합니다.
