...
## 2024-08-09 - [Bash 성능 개선] 정적 문자열 치환 루프 내장 기능으로 언롤링(Unrolling)
**Learning:** Bash 스크립트에서 정적인 텍스트 치환을 위해 `for` 루프 내부에서 `printf`와 같은 빌트인을 반복 호출하면 서브쉘과 파싱 오버헤드가 누적되어 성능이 크게 저하됩니다. 루프를 풀어서(Unrolling) 파라미터 확장을 직접 나열하는 방식이 반복 호출보다 약 3~4배 더 빠릅니다.
**Action:** 고정된 횟수만큼 반복되는 문자열 치환 작업(예: 제어 문자 이스케이프)은 루프를 사용하는 대신 `value="${value//pattern/replacement}"` 형태의 순수 파라미터 확장을 시퀀셜하게 직접 작성하여 프로세스 호출 비용을 없애고 성능을 최적화합니다.
