...
## 2024-07-17 - Early Return on Heavy CLI Processing
**Learning:** Implementing early returns before executing heavy network or processing tools (like yt-dlp) prevents expensive redundant execution. Crucially, when checking file existence, always use `[ -f "$FILE" ] && [ -s "$FILE" ]` instead of just `-s`, because directories evaluate to true with `-s` resulting in false-positive skips.
**Action:** Always implement explicit early return checks combining `-f` and `-s` prior to external downloads or heavy processing tasks.
