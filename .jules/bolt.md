...
## 2024-08-04 - yt-dlp concurrent fragments optimization
**Learning:** Using `yt-dlp` for DASH/HLS streams without concurrency leads to significant network I/O bottlenecks.
**Action:** Always add `--concurrent-fragments N` (e.g., `--concurrent-fragments 4`) to `yt-dlp` commands to parallelize fragment fetching and significantly improve download speeds.
