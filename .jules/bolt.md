...
## 2025-07-16 - [Bash 성능 개선] 무거운 외부 네트워크 호출 전 Early return 적용
**Learning:** `yt-dlp`와 같은 외부 네트워크 도구는 실행 시 높은 오버헤드가 발생하므로 불필요한 중복 실행을 피해야 합니다. 특히 `-s` 플래그는 디렉터리일 경우 true를 반환할 수 있으므로, 반드시 `-f` 플래그와 함께 사용하여 파일인지 확인해야 합니다.
**Action:** 비싼 연산이나 네트워크 I/O를 발생시키는 CLI 도구를 호출하기 전에는 `[ -f "$OUTPUT" ] && [ -s "$OUTPUT" ]` 형태의 Early return 검증 로직을 추가하여 중복 작업을 방지하고 성능을 향상시킵니다.
