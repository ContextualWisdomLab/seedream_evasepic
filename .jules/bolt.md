...
## 2024-08-29 - [Bash 성능 개선] 유틸리티 함수 내 서브쉘(subshell) 오버헤드 방지
**Learning:** Bash 스크립트에서 유틸리티 함수의 출력을 캡처하기 위해 서브쉘(`result="$(my_func)"`)을 사용하면 프로세스 분기(forking)로 인해 심각한 성능 저하가 발생하며, 호출 횟수에 비례하여 병목 현상이 심해집니다.
**Action:** 유틸리티 함수가 선택적인 출력 변수 이름을 인자로 받도록 수정하세요. 변수 이름이 제공된 경우 `printf -v <var>`를 사용해 결과를 변수에 직접 할당하고, 제공되지 않은 경우 기본 출력(`printf`)을 사용하도록 구현하면 하위 호환성을 유지하면서 성능을 최적화할 수 있습니다. 단, 호출부에서 `printf -v`를 사용하기 전에 출력 변수(예: `safe_value=""`)를 명시적으로 초기화하여 SC2154와 같은 정적 분석 오류를 방지하세요.
