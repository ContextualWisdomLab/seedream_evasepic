...
## 2024-05-24 - Bash 서브쉘 호출로 인한 프로세스 포크 오버헤드 최적화
**Learning:** Bash 유틸리티 함수(예: `terminal_safe_text`)에서 결과를 반환할 때 `$(...)` 형태의 서브쉘 캡처 방식을 사용할 경우, 함수가 호출될 때마다 추가 프로세스 포크가 발생하여 큰 성능 오버헤드가 발생한다.
**Action:** 유틸리티 함수의 인자로 결과값을 저장할 변수명을 추가로 받게 하고 내장 명령어인 `printf -v`를 사용해 직접 변수에 결과를 할당하는 패턴을 적용하여, 이전 버전과 하위 호환성을 유지하면서 불필요한 포크 오버헤드를 제거한다.
