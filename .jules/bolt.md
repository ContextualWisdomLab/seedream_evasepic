...
## 2025-01-20 - [Bash 문자열 조작 및 셸 서브프로세스 성능 최적화]
**Learning:** Bash에서 서브셸 `$(...)`을 사용하여 함수 출력을 캡처하는 것은 프로세스 포크로 인해 상당한 성능 저하를 초래합니다. 또한 내장 명령 `printf -v`와 루프를 사용하는 것보다 직접적인 매개변수 확장(parameter expansion, e.g. `${var//pattern/replacement}`)을 하드코딩하여 사용하는 것이 문자열 조작에 있어 훨씬 빠릅니다.
**Action:** API의 하위 호환성을 깨지 않고 서브셸을 피하려면, 선택적 출력 변수 이름을 인수로 받아 `printf -v`를 사용해 결과를 해당 변수에 직접 할당하고, 인수가 생략된 경우 표준 출력(`printf`)으로 폴백(fallback)하도록 구현해야 합니다. 셸 스크립트에서 텍스트 처리가 필요한 부분에서는 정적인 매개변수 확장을 활용하여 성능을 개선할 수 있습니다.
