...
## 2024-09-09 - [Bash 루프 언롤링 최적화]
**Learning:** 텍스트 처리 함수에서 bash 루프 내부에 `printf -v`를 반복적으로 사용하면 성능에 심각한 오버헤드가 발생합니다.
**Action:** 루프를 정적이고 하드코딩된 파라미터 확장(예: `value=${value//pattern/replacement}`)으로 언롤링하여 텍스트 정제 루프의 런타임 오버헤드를 방지하세요.
