...
## 2026-07-18 - bash 다중 printf 최적화
**Learning:** 셸 스크립트에서 에러 메시지나 여러 줄의 안내문을 출력할 때, `printf` 내장 명령을 연속적으로 여러 번 호출하면 그만큼 셸 내부 처리 오버헤드가 발생한다.
**Action:** 단일 `printf` 문과 하나의 다중 줄 포맷 문자열을 사용하여 출력을 통합함으로써 불필요한 내장 명령 실행 오버헤드를 줄인다.
