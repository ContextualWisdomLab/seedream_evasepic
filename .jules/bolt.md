...
## 2025-02-19 - [Bash 성능 개선] 정적 문자열 치환을 위한 내장 루프 언롤링 최적화
**Learning:** 쉘 스크립트에서 단순한 텍스트 치환(예: 제어 문자 처리)을 위해 루프 내에서 `printf`와 같은 내장 명령어를 반복 호출하면 서브쉘/프로세스 오버헤드가 발생하여 성능이 크게 저하됩니다.
**Action:** 루프 내에서 정적인 문자열 패턴을 반복 치환하는 경우, 해당 루프를 풀어서(Unrolling) Bash 파라미터 확장(예: `value=${value//$'\001'/\x01}`)을 순차적으로 직접 작성하여 성능을 최대화합니다.
