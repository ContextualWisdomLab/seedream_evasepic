...
**Learning:** 스크립트 내에서 외부 명령어(bc, awk 등)를 여러 번 호출하여 계산하는 매커니즘은 서브쉘과 프로세스 생성 오버헤드를 유발합니다.
...
## 2024-05-24 - [Bash 성능 개선] 서브셸 캡처 오버헤드 최적화
**Learning:** Bash에서 함수 출력을 서브셸(예: `result="$(my_func)"`)로 캡처하면 프로세스 포크 오버헤드가 발생하여 성능이 저하됩니다. 이 오버헤드는 스크립트에서 빈번하게 호출될 때 더 크게 누적됩니다.
**Action:** 서브셸을 피하고 API 하위 호환성을 깨지 않기 위해, 함수에 선택적 출력 변수명을 인자로 받고 `printf -v`를 사용해 결과를 변수에 직접 할당합니다. 인자가 제공되지 않은 경우에는 기본 출력(예: `printf`)으로 처리합니다.
