...
## 2025-02-19 - [Bash 성능 개선] yt-dlp 중복 실행 방지를 위한 조기 반환 추가
**Learning:** yt-dlp와 같은 외부 네트워크 도구를 불필요하게 여러 번 실행하는 것은 큰 오버헤드를 유발합니다. 단순 `-s` 대신 `[ -f "$OUTPUT" ] && [ -s "$OUTPUT" ]`을 사용하여 디렉터리 오탐을 방지하면서 조기 반환을 처리하면 성능을 크게 향상시킬 수 있습니다.
**Action:** 무거운 외부 도구를 실행하기 전에 결과 파일이 이미 존재하는지(`[ -f ] && [ -s ]`) 검사하는 조기 반환 로직을 추가하여 중복 실행을 피합니다.
