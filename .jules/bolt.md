...
## 2024-09-24 - [Bash 성능 개선] 반복 호출되는 함수 내 printf -v 문자열 포맷팅 오버헤드 제거
**Learning:** Bash 내장 명령어인 `printf`는 서브셸 프로세스를 생성하지는 않지만, `terminal_safe_text`처럼 빈번히 호출되는 함수 내의 루프문 안에서 반복 실행할 경우 내부 문자열 해석 및 포맷팅 오버헤드가 크게 누적되어 성능이 저하됩니다.
**Action:** 스크립트 로드 시점에 전역 배열에 필요한 변환 값(C0, C1 제어 문자 등)을 미리 계산(Precompute)하여 캐싱해 두고, 함수 내에서는 단순히 배열의 값을 참조하여 파라미터 확장을 수행하도록 최적화함으로써 실행 시간을 대폭 단축시킵니다.
