...
## 2026-09-22 - [터미널 텍스트 안전 처리 속도 최적화]
**Learning:** Bash에서 수십 개의 매개변수 확장(parameter expansion)을 수행하는 것은 I/O에 비해 눈에 띄는 오버헤드를 발생시킵니다. 터미널 출력 이스케이프와 같이 입력의 대부분이 인쇄 가능한 ASCII인 경우, `[!$'\x20'-$'\x7E']`와 같은 정규식 매칭을 통해 빠른 경로(fast-path)를 제공하면 불필요한 루프를 건너뛰어 성능이 20배 이상 크게 향상됩니다.
**Action:** 순수 텍스트 데이터를 처리하는 무거운 문자열 치환 로직 앞에는 항상 입력이 안전한 문자열인지 검사하는 fast-path를 두어 오버헤드를 줄입니다.
