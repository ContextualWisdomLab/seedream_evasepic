...
## 2026-08-04 - Optimize network and initialization overhead with early returns
**Learning:** Attempting to download external assets or invoking heavy initialization routines (like checking for dependencies and running yt-dlp) when the output file already exists wastes time and network resources.
**Action:** Implement an early return check using `[ -f "$OUTPUT" ] && [ -s "$OUTPUT" ] && [ ! -L "$OUTPUT" ]` before executing heavy operations or checking for heavy dependencies, ensuring that redundant operations are skipped safely.
