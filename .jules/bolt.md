...
## 2024-05-24 - Unrolling string manipulation loops in Bash
**Learning:** In Bash, repetitive string manipulations across a known, fixed set of characters (such as C0/C1 control codes) using a `for` loop with `printf -v` incurs significant execution overhead.
**Action:** Unroll these loops into direct parameter expansions (e.g., `${var//pattern/replacement}`) to eliminate loop and built-in invocation overhead.
