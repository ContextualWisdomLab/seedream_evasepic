...
## 2024-09-01 - [Bash 성능 개선] 서브셸 생성 방지를 통한 문자열 반환 최적화
**Learning:** Bash에서 함수 결과를 반환할 때 서브셸(`result="$(my_func)"`)을 사용하면 프로세스 포크 오버헤드가 발생하여 성능이 크게 저하됩니다.
**Action:** 유틸리티 함수에서 서브셸 없이 값을 반환하려면 선택적 출력 변수 이름을 인자로 받고 `printf -v`를 사용하여 해당 변수에 결과를 직접 할당(예: `printf -v "$out_var"`)하도록 최적화합니다. 기존 API 호환성을 위해 인자가 생략된 경우 `printf`로 표준 출력하도록 폴백(fallback)을 제공합니다.
