...
## 2026-10-27 - Bash 내장 파라미터 확장을 활용한 외부 프로세스 오버헤드 제거
**Learning:** 단순 문자열 치환(예: 공백 제거)을 위해 외부 명령어인 `tr`을 파이프로 연결하여 호출하면 서브쉘 및 프로세스 포크 오버헤드가 발생하여 성능이 저하됩니다.
**Action:** `tr`과 같은 외부 명령어 대신 Bash 내장 파라미터 확장(예: `${VAR//[[:space:]]/}`)을 활용하여 순수 Bash 기능만으로 문자열을 조작하여 성능을 향상시킵니다.
