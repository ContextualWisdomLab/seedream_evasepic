...
## $(date +%Y-%m-%d) - [Bash 성능 개선] 서브셸 없는 함수 결과 반환
**Learning:** Bash 스크립트에서 함수 실행 결과를 `var="$(func)"`와 같이 명령 치환(Command Substitution) 서브셸을 통해 변수에 할당하면, 프로세스 포크 오버헤드로 인해 빈번히 호출되는 로직에서 성능 저하가 발생합니다.
**Action:** 성능에 민감하고 반복 호출되는 함수는 출력할 변수 이름을 인자로 받아 `printf -v <var_name>`을 이용해 직접 변수에 값을 할당하도록 설계하여 서브셸 생성 비용을 피합니다.
