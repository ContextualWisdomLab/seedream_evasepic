...
## 2024-08-28 - [Subshell Optimization]
**Learning:** In Bash, capturing function output via subshells (e.g., `result="$(my_func)"`) introduces significant performance overhead due to process forking.
**Action:** Accept an optional output variable name as an argument and use `printf -v` to assign the result directly to that variable, falling back to standard output (e.g., `printf`) if the argument is omitted.
