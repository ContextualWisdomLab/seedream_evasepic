...
## 2024-11-20 - Unroll loop for bash parameter expansion in terminal-output.sh
**Learning:** In Bash, executing built-in commands like `printf -v` repeatedly inside a loop introduces significant performance overhead, especially in utility functions that are called frequently on large datasets. Additionally, capturing function output via subshells (e.g., `result="$(my_func)"`) introduces significant process forking overhead.
**Action:** Unroll loops into static, hardcoded parameter expansions (e.g., `value=${value//pattern/replacement}`) and use `printf -v` with an optional output variable argument to assign the result directly, avoiding the need for subshells and repeated built-in invocations in the fast path.
