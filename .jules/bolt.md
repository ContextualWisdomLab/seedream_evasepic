...
## 2025-06-25 - [Bash 성능 개선] 다운로드 전 로컬 캐시 확인을 통한 조기 종료(Early Return) 패턴
**Learning:** 네트워크 통신이 발생하거나 외부 CLI 툴(yt-dlp 등)을 호출하는 스크립트에서는, 의도한 결과 파일이 이미 로컬에 존재할 경우 전체 프로세스를 조기 종료하는 방식이 성능에 큰 이점을 줍니다. 특히 스크립트 재실행 시 중복 작업을 피할 수 있습니다. 주의할 점은 Bash 파일 확인 시 단순 `-s` 플래그는 디렉터리에 대해서도 참(true)을 반환하므로 파일 여부 확인을 위해 반드시 `[ -f "$FILE" ] && [ -s "$FILE" ]` 형태로 함께 사용해야 한다는 점입니다.
**Action:** 외부 프로세스나 네트워크 호출을 하는 Bash 스크립트를 최적화할 때는 파일이 이미 존재하는지 먼저 확인하여 빠르게 `exit 0` 처리하는 '조기 종료(Early Return)' 로직을 추가합니다.
