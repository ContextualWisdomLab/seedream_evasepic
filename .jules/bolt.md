...
## 2026-08-04 - [Bash 성능 개선] yt-dlp DASH/HLS 스트림 다운로드 속도 최적화
**Learning:** `yt-dlp`를 사용하여 DASH/HLS 스트림을 다운로드할 때 단일 연결을 사용하면 네트워크 I/O 병목이 발생하여 다운로드 속도가 제한됩니다.
**Action:** `yt-dlp` 명령어에 `--concurrent-fragments N` (예: `--concurrent-fragments 4`) 옵션을 추가하여 프래그먼트 다운로드를 병렬화함으로써 다운로드 속도를 대폭 향상시키고 네트워크 I/O 병목 현상을 줄일 수 있습니다.
