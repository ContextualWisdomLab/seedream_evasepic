...
## 2026-07-19 - [Bash 성능 개선] 무거운 네트워크 작업 전 early return 적용
**Learning:** yt-dlp와 같은 외부 네트워크 도구를 불필요하게 실행하면 시스템 및 네트워크 리소스가 낭비됩니다. 결과 파일이 이미 존재하는지 미리 확인하면 실행 비용을 크게 줄일 수 있습니다.
**Action:** 무거운 처리 도구를 호출하기 전에 항상 `[ -f "$OUTPUT" ] && [ -s "$OUTPUT" ]` 형태의 early return을 구현하여 이미 존재하는 파일에 대한 중복 처리를 방지합니다. (`-s`만 쓰면 디렉토리에서 오탐할 수 있으므로 `-f`도 함께 사용합니다.)
