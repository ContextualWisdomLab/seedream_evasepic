...
## 2025-02-19 - [Bash 성능 개선] 외부 네트워크 요청 전 조기 반환(Early Return) 체크로 불필요한 실행 방지
**Learning:** `yt-dlp`와 같이 무거운 외부 네트워크나 처리 도구를 호출하기 전에, 대상 파일이 이미 존재하고 크기가 0이 아닌지 확인하지 않으면 불필요한 중복 실행(expensive redundant execution)이 발생할 수 있습니다.
**Action:** 외부 도구를 호출하기 전에 항상 `[ -f "$OUTPUT" ] && [ -s "$OUTPUT" ]` 형태의 조건문으로 파일 존재 여부와 내용 유무를 검증하여 조기 반환(early return)을 구현합니다. 단일 `-s` 플래그는 디렉터리의 경우 true로 평가될 수 있으므로 반드시 `-f`와 함께 사용해야 합니다.
