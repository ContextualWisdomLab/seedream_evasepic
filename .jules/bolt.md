...
## 2023-10-27 - Bash Parameter Expansion vs Loops
**Learning:** Repetitive string manipulations (like neutralizing C0/C1 control codes) inside a Bash `for` loop with `printf` incurs significant execution overhead.
**Action:** Unroll these loops and use direct parameter expansions (e.g., `${var//pattern/replacement}`) to eliminate loop and built-in invocation overhead. This reduces execution time significantly (e.g., from ~1.7ms to ~0.5ms per call).
