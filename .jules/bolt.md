...
## 2025-02-19 - [Bash 성능 개선] 루프와 printf 내장 명령어를 피하여 문자열 치환 최적화
**Learning:** 셸 스크립트에서 변수 내 다수의 문자를 치환할 때, `for` 루프 안에서 `printf -v`를 반복 호출하는 방식은 상당한 오버헤드를 발생시킵니다.
**Action:** C0, C1 제어 문자 등 다수의 고정된 문자를 치환해야 할 경우, 루프 대신 Bash 내장 파라미터 확장 기능(`value=${value//$'\001'/\\x01}` 등)을 하드코딩하여 직접 수행하면 문자열 치환 속도를 약 5~7배 이상 크게 향상시킬 수 있습니다.
