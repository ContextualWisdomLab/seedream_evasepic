...
## 2024-07-23 - [Bash 성능 개선] 외부 도구 실행 전 조기 반환(Early Return) 적용
**Learning:** `yt-dlp`와 같이 무거운 외부 네트워크나 처리 CLI 도구를 실행하기 전에 결과 파일이 이미 존재하는지 확인하지 않으면 비용이 큰 중복 실행이 발생합니다. 단순한 파일 크기 확인(`-s`)은 디렉터리의 경우에도 참(true)으로 평가될 수 있어 잘못된 스킵(false-positive skip)을 유발할 수 있습니다.
**Action:** 쉘 스크립트에서 비용이 큰 도구를 호출하기 전에는 항상 `[ -f "$OUTPUT" ] && [ -s "$OUTPUT" ]` 형태의 엄격한 조기 반환(early return) 검사를 구현하여 불필요한 실행 오버헤드를 방지해야 합니다.
