...
## 2025-02-19 - [Bash 성능 개선] 루프와 printf를 활용한 문자열 치환 오버헤드 최적화
**Learning:** Bash 스크립트에서 문자열을 치환할 때 `for` 루프 내부에서 `printf -v`와 같은 빌트인 명령어를 반복 호출하면 반복 횟수만큼 오버헤드가 발생하여 스크립트 실행 시간이 지연됩니다.
**Action:** 고정된 패턴(제어 문자 등)에 대한 치환이 필요한 경우 루프를 풀어 직접 파라미터 확장(예: `value=${value//$'\\001'/\\x01}`)을 전개하여 실행 오버헤드를 대폭 줄입니다.
