...
## 2024-05-20 - Unrolling Loop String Interpolations in Bash
**Learning:** Using `for` loops combined with `printf -v` to dynamically compute search/replace patterns and perform parameter expansions (`value=${value//"$control"/"$replacement"}`) in Bash introduces significant performance overhead compared to statically unrolled parameter expansions.
**Action:** Unroll loops into static parameter expansions (`value=${value//$'\001'/\x01}`) when optimizing text processing functions in Bash.
