...
## 2024-05-15 - [성능 개선] Bash 서브셸 캡처를 전역 변수 할당으로 대체
**Learning:** Bash에서 함수 출력을 서브셸(Subshell)을 통해 캡처(`result="$(my_func)"`)하는 것은 프로세스 포크(process forking) 오버헤드로 인해 성능 저하를 유발합니다. 고성능이 요구되는 코드에서는 서브셸 대신 함수가 전역 변수(`RET`)에 출력을 직접 할당하도록 변경하면 눈에 띄는 성능 향상을 얻을 수 있습니다.
**Action:** 서브셸을 사용하여 함수 반환값을 얻는 패턴을 전역 변수 할당 패턴(`my_func; result="$TERMINAL_SAFE_RET"`)으로 교체하여 성능을 향상시킵니다.
## 2024-05-15 - [성능 개선] Bash 서브셸 캡처를 printf -v를 사용한 참조 전달로 대체
**Learning:** Bash에서 함수 출력을 서브셸(Subshell)을 통해 캡처(`result="$(my_func)"`)하는 것은 프로세스 포크(process forking) 오버헤드로 인해 성능 저하를 유발합니다. 전역 변수 할당은 API 파괴를 유발할 수 있으므로, `printf -v "$output_variable" "%s" "$value"` 방식을 사용하여 출력 변수 이름을 참조(reference)로 전달하는 것이 기존 호환성을 유지하면서도 안전하게 서브셸을 제거하는 최고의 방법입니다.
**Action:** 서브셸 캡처(`$(...)`) 패턴을 제거해야 할 경우, 출력 변수 이름을 함수의 인자로 전달하고 내부에서 `printf -v "$var_name"`을 사용하여 오버헤드 없이 값을 반환하도록 코드를 개선합니다.
