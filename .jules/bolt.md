...
## 2024-07-13 - [Bash 성능 개선] yt-dlp 네트워크 호출 및 불필요한 프로세스 실행 방지 (Early Return)
**Learning:** 네트워크 통신이나 외부 프로세스(`yt-dlp` 등)를 호출하는 작업은 매우 높은 오버헤드를 유발합니다. 스크립트 실행 전에 결과물이 이미 존재하고 유효한지 확인하지 않으면 불필요하게 시간과 자원을 낭비하게 됩니다. 특히 단순한 `-s` 옵션은 디렉터리에도 참(true)으로 평가될 수 있으므로 주의가 필요합니다.
**Action:** 무거운 외부 프로세스를 호출하기 전에 항상 `[ -f "$OUTPUT" ] && [ -s "$OUTPUT" ]` 조건을 사용하여 대상 파일이 존재하고 비어 있지 않은 경우 즉시 종료(early return)하도록 스크립트를 최적화해야 합니다.
