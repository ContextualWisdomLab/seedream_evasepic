...
## 2024-07-26 - [Bash 성능 개선] 제어 문자 치환 루프 언롤링 최적화
**Learning:** Bash 스크립트 내에서 고정된 제어 문자(C0, C1) 셋을 `for` 루프와 `printf -v` 등을 이용해 동적으로 치환하면, 루프 제어와 내장 함수 호출에 따른 오버헤드가 누적되어 처리 시간이 크게 지연됩니다.
**Action:** 알려져 있고 고정된 문자에 대한 치환 작업은 루프를 제거하고 직접 파라미터 확장(예: `value=${value//패턴/치환열}`)을 언롤링하여 작성하면 반복 및 함수 호출 비용을 없애 성능을 대폭 향상시킬 수 있습니다.
