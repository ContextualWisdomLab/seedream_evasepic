...
## 2024-02-19 - [Bash 성능 개선] 다중 echo 호출을 단일 printf로 병합
**Learning:** 스크립트에서 동일한 파일에 여러 줄을 기록할 때 여러 번의 `echo`를 `{ ... } > file` 블록으로 묶어 사용하는 것보다 단일 `printf` 명령을 사용하는 것이 함수 호출 오버헤드를 줄여 실행 속도를 높입니다.
**Action:** 앞으로 셸 스크립트에서 여러 줄의 텍스트를 파일에 출력할 때는 다중 `echo`나 서브셸을 피하고, 단일 `printf` 포맷 문자열(`printf "...\n" "$VAR1" "$VAR2"`)을 우선적으로 사용하여 성능을 향상시킵니다.
