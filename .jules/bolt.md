...
## 2024-07-28 - 서브셸 캡처를 printf -v 로 대체하여 오버헤드 제거
**Learning:** Bash 스크립트에서 유틸리티 함수의 결과를 캡처하기 위해 서브셸(`$()`)을 사용하면 매 호출마다 새로운 프로세스가 포크되어 상당한 성능 오버헤드가 발생합니다.
**Action:** 외부 명령어가 아닌 순수 Bash 유틸리티 함수에서는 선택적인 출력 변수명을 인자로 받아 `printf -v`를 사용해 결과를 변수에 직접 할당하도록 수정하여 프로세스 포크 비용을 제거합니다. 하위 호환성을 위해 인자가 없을 때는 기존처럼 표준 출력으로 결과를 내보냅니다.
