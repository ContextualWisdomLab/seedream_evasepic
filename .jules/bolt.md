...
## 2024-09-15 - [Bash 성능 개선] printf -v 변환 반복문 언롤링을 통한 오버헤드 방지
**Learning:** 쉘 스크립트의 루프 내부에서 `printf -v` 등 포맷팅 내장 명령어를 반복적으로 호출하면 무시할 수 없는 성능 오버헤드가 발생합니다.
**Action:** 알려진 범위의 고정된 문자 세트(예: C0, C1 제어 문자)를 다룰 때는 루프를 사용하는 대신 `value=${value//패턴/치환}` 형태의 하드코딩된 Bash 내장 파라미터 확장을 사용하여 프로세싱 비용을 줄이고 성능을 대폭 향상시킵니다.
