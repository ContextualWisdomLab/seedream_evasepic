...
## 2025-02-23 - Bash Loop Overhead in String Manipulation
**Learning:** Built-in command invocations (like `printf -v`) inside `for` loops carry significant execution overhead in Bash. Unrolling repetitive string operations over a fixed set of characters into direct parameter expansions avoids this overhead.
**Action:** When performing string manipulations across a known, static set of values (e.g., C0/C1 control character neutralization), explicitly unroll the variable substitutions rather than looping to maximize execution speed.
