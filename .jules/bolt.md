...
## 2025-02-19 - [Bash 성능 개선] 무거운 외부 명령어 중복 실행 방지 (Early Return)
**Learning:** `yt-dlp`와 같은 무거운 네트워크 통신 및 파일 처리 명령어를 실행할 때, 이미 결과물이 존재하는지 확인하지 않으면 불필요한 실행 오버헤드 및 네트워크 대역폭 낭비가 발생합니다. 또한 파일 존재 여부를 검사할 때 `-s` 플래그만 사용하면 디렉터리에도 참으로 평가되어 오작동할 수 있습니다.
**Action:** 무거운 작업을 수행하기 전에 항상 조기 반환(Early Return) 로직을 추가하며, 파일 검증 시에는 반드시 `[ -f "$OUTPUT" ] && [ -s "$OUTPUT" ]`을 사용하여 올바르게 판단합니다.
