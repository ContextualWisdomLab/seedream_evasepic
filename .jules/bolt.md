...
## 2024-09-24 - [Bash 성능 개선] 반복 호출되는 함수 내 printf -v 오버헤드 제거
**Learning:** `terminal_safe_text`와 같이 반복적으로 호출될 수 있는 함수 내에서 `printf -v`를 루프문 안에서 반복 실행하면 프로세스 생성 및 문자열 포맷팅 오버헤드가 크게 누적되어 성능이 저하됩니다.
**Action:** 스크립트 로드 시점에 전역 배열에 필요한 변환 값(C0, C1 제어 문자 등)을 미리 계산(Precompute)하여 저장해 두고, 함수 내에서는 단순히 배열의 값을 참조하여 파라미터 확장을 수행하도록 최적화함으로써 실행 시간을 대폭 단축시킵니다.
