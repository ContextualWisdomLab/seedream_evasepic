...
## 2026-08-04 - 구체적인 CLI 오류 메시지와 인자 검증 순서
**Learning:** CLI 사용자가 필수 인자를 누락했을 때, 포괄적인 오류("Missing required argument(s)")보다 정확히 어떤 인자가 누락되었는지 명시해주는 것("<url>이 누락됨")이 디버깅과 사용자 경험에 크게 기여함. 또한, 필수 인자의 유효성을 선택 인자보다 먼저 검증해야 사용자에게 우선순위에 맞는 정확한 오류 메시지를 전달할 수 있음.
**Action:** 스크립트 작성 시 모든 필수 인자를 개별적으로 검사하고 구체적으로 오류를 반환하며, 필수 인자 검증 로직을 선택 인자 검증 로직 상단에 위치시킨다.
