...
## 2024-10-21 - 터미널에서의 파일 크기 출력 가독성 개선
**Learning:** CLI 스크립트에서 파일 크기를 단순 바이트(bytes) 단위로 출력하면, 큰 파일의 경우 사용자가 직관적으로 크기를 가늠하기 어렵습니다. 사용자 경험을 높이기 위해 KB, MB, GB 등의 사람이 읽기 쉬운(Human-readable) 포맷으로 변환하여 제공하는 것이 훨씬 효과적입니다.
**Action:** `download-reference.sh` 스크립트에서 다운로드된 파일의 크기를 출력할 때 `awk`를 사용하여 조건부 포맷팅을 적용, 바이트 수를 읽기 편한 단위로 변환해 출력하도록 수정했습니다. 앞으로 출력되는 수치 데이터는 항상 사용자가 직관적으로 이해할 수 있는 단위로 표시해야 합니다.
