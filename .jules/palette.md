...
## 2024-05-24 - CLI 에러 메시지 가독성 개선
**Learning:** CLI 스크립트에서 에러 상태와 해결 가능 단계를 시각적으로 명확하게 구분하지 않으면 사용자의 인지 부하가 가중된다는 점을 발견했습니다.
**Action:** 앞으로 모든 스크립트 에러 출력 시 오류 원인은 빨간색(Red)으로, 조치 가능한 해결책(설치 명령어 등)은 청록색(Cyan)으로 명확히 구분하여 인지 부하를 줄여야 합니다.
