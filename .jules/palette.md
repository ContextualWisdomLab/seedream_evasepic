...
## 2025-02-28 - Explicit sub-dependency validation in CLI UX
**Learning:** CLI 스크립트에서 메인 의존성(예: ffmpeg) 뿐만 아니라 백그라운드에서 사용되는 보조 도구(예: ffprobe)의 존재 여부도 명시적으로 확인해야 합니다. 그렇지 않으면 모호한 다운스트림 에러(예: 0s duration)가 발생하여 사용자가 문제 원인을 파악하기 어렵습니다.
**Action:** 모든 외부 도구를 실행하기 전에 `[ -x "$DEPENDENCY" ]`를 사용하여 실행 가능 여부를 개별적으로 검증하고 명확한 안내 메시지를 출력하도록 합니다.
