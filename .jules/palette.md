...
## 2025-03-01 - 보조 도구(ffprobe 등)에 대한 명시적 의존성 검증 추가
**Learning:** `ffprobe` 같은 보조 도구가 누락되었을 때 이를 사전에 검증하지 않으면 메타데이터 파싱이 조용히 실패하고, 결국 하위 로직에서 혼란스러운 에러("video duration must be a positive number")가 발생함을 배웠습니다.
**Action:** 메인 로직을 실행하기 전에 항상 모든 외부 의존성(예: `ffmpeg`와 `ffprobe` 모두)의 존재와 실행 가능 여부를 명시적으로 검증하여 사용자에게 명확하고 조치 가능한 에러 메시지를 제공해야 합니다.
