...
## 2024-10-25 - [CLI UX] 명확한 필수 인자 누락 에러 메시지
**Learning:** CLI 사용자가 필수 인자를 누락했을 때, 단순히 'Missing required argument(s)'라고 안내하면 어떤 인자가 빠졌는지 바로 파악하기 힘들어 사용성이 저하됨.
**Action:** 여러 인자가 필요한 스크립트에서는 먼저 순서대로 각 인자를 개별 검증하고, 정확히 어떤 인자(예: `<url>`)가 누락되었는지 명시적으로(targeted) 오류 메시지에 출력하도록 수정. 추가로, 필수 인자 검증은 항상 선택적 인자 유효성 검사보다 먼저 실행되도록 순서를 보장해야 함.
