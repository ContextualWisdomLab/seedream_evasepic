...
## 2025-03-01 - [의존성 자동 설치 후 PATH 검증 로직 추가]
**Learning:** 시스템 의존성(예: yt-dlp)을 자동 설치한 후 사용자 환경의 PATH 설정 문제로 인해 명령어를 즉시 실행할 수 없는 상황이 발생할 수 있음을 깨달았습니다. 성공적인 설치 메시지 후 알 수 없는 오류로 실패하면 사용자 경험이 크게 저하됩니다.
**Action:** 외부 도구 자동 설치 로직 직후에 해당 도구가 PATH 환경 변수에 포함되어 올바르게 인식되는지 다시 확인(command -v)하고, 그렇지 않은 경우 원인(PATH 문제)과 해결책(수동 설치 명령어 등)을 색상(빨간색, 청록색)으로 구분하여 명확하게 안내합니다.
