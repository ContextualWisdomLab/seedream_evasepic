...
## 2024-05-25 - CLI Error Message Clarity and Parsing Precedence
**Learning:** Generic error messages (like 'Missing required argument(s)') provide poor UX because the user must guess which parameter they omitted. Furthermore, validating optional arguments before ensuring required ones are present can lead to confusing error precedence where a user is scolded for an invalid optional value when they completely forgot the primary required input.
**Action:** Always provide explicit, targeted feedback that clearly identifies exactly which argument is missing (e.g., 'Missing required argument: <url>'). In CLI scripts, strictly validate and report missing required arguments before evaluating any optional arguments.
