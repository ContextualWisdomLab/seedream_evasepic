...
## 2025-02-28 - CLI 의존성 자동 설치 후 PATH 재검증
**Learning:** CLI 스크립트가 누락된 의존성을 자동 설치하려고 시도한 후, 설치된 실행 파일이 시스템의 `$PATH`에 존재하지 않아 스크립트가 계속 실패하는 경우가 있음을 발견했습니다. 사용자는 설치가 성공했다고 생각하지만 실제로는 실행하지 못해 혼란을 겪을 수 있습니다.
**Action:** 의존성을 자동 설치한 직후에는 항상 `$PATH`에서 실행 파일의 존재 여부를 다시 검증합니다. 여전히 접근할 수 없는 경우, 사용자에게 PATH를 수정하거나 수동으로 설치하라는 구체적인 안내 메시지를 제공하여 문제를 해결할 수 있도록 합니다.
