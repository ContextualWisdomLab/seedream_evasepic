...
## 2026-08-04 - Specific Missing Argument Errors in CLI
**Learning:** Generic "Missing required argument(s)" errors force users to cross-reference usage docs to figure out exactly what they forgot, slowing them down. Specifying the exact missing argument in the error message itself makes the feedback immediately actionable and reduces cognitive load.
**Action:** When validating inputs, identify and state exactly which field/argument is missing rather than grouping multiple validation failures into a generic error message.
