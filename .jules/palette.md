...
## 2025-03-01 - [CLI UX] Example 명령어 구문 강조(Syntax Highlighting)
**Learning:** CLI 도움말이나 오류 메시지에서 실행 가능한 예시(Example) 명령어를 단색 텍스트로만 제공하면, 주변의 설명 텍스트와 구분되지 않아 사용자가 한눈에 파악하기 어렵습니다.
**Action:** `--help` 및 오류 메시지의 `Example:` 섹션에서 실제 실행 가능한 명령어 부분에만 ANSI 색상 코드(CYAN)를 적용하여 시각적 대비를 주었고, 복사/붙여넣기 및 인지 속도를 향상시켰습니다.
