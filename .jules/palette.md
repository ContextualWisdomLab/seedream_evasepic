...
## 2025-02-28 - 파일 크기를 사람이 읽기 쉬운 형식으로 변환하여 출력
**Learning:** 다운로드 스크립트에서 파일 크기를 단순히 바이트(bytes) 단위로만 출력하면 사용자가 파일의 실제 크기를 직관적으로 파악하기 어렵다는 점을 확인했습니다.
**Action:** `download-reference.sh` 스크립트에서 파일 크기(bytes)를 KiB, MiB, GiB 등의 사람이 읽기 쉬운 단위로 변환하여 출력하도록 개선했습니다.
