...
## 2026-07-26 - [CLI 에러 메시지 색상 분리]
**Learning:** CLI 스크립트에서 에러 원인과 실행 가능한 해결 방법(actionable steps)을 시각적으로 분리(Red vs Cyan)하는 것이 사용자의 인지 부하를 크게 줄여준다는 것을 확인했습니다. 또한 테스트 스크립트에 이 내용을 추가하여 회귀를 방지할 수 있음을 확인했습니다.
**Action:** 향후 모든 CLI 스크립트에서 에러 상태는 Red, 조치 가능한 대안은 Cyan으로 엄격하게 분리하여 색상을 적용하고, 이에 대한 테스트 코드를 작성합니다.
