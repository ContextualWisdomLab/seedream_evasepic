...
## 2026-07-22 - [CLI 의존성 사전 검증 추가]
**Learning:** 외부 도구(ffmpeg, ffprobe 등)에 의존하는 CLI 스크립트에서 메인 로직 실행 전에 모든 의존성 도구의 존재 여부를 명시적으로 확인하지 않으면, 사용자가 원인을 알기 힘든 모호한 에러에 직면하게 됩니다.
**Action:** 스크립트 작성 시 사용되는 모든 주요 외부 의존성에 대해 명시적인 존재 및 실행 가능 여부(예: `[ ! -x "$FFPROBE" ]`) 검증 로직을 도입하고, 누락 시 명확하고 조치 가능한 에러 메시지를 제공하도록 합니다.
