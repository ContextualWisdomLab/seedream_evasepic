...
## 2026-09-16 - [CLI UX Enhancement] 파일 크기를 사람이 읽기 쉬운 형식으로 표시
**Learning:** CLI 도구에서 파일 크기를 바이트 단위로만 출력하면, 용량이 커질수록 사용자가 실제 크기를 직관적으로 파악하기 어렵다는 점을 배웠습니다. 이를 사람이 읽기 쉬운 형식(KiB, MiB, GiB)으로 변환하면 인지적 부하를 줄일 수 있습니다.
**Action:** `download-reference.sh` 스크립트에서 다운로드 완료 후 파일 크기를 출력할 때, `awk`를 사용하여 이진 접두어(Binary prefix) 기반의 읽기 쉬운 포맷(KiB, MiB, GiB)으로 변환하여 출력하도록 개선했습니다. 또한 제한된 PATH에서 테스트가 동작하도록 `test_cli_ux.sh`에 `awk` 및 `wc` 심볼릭 링크를 추가했습니다.
