...
## 2023-11-20 - [CLI UX Enhancement] 의존성 검증 오류 메시지 개선
**Learning:** 쉘 스크립트 기반 CLI 도구에서 외부 의존성(예: ffprobe)이 누락되었을 때, 이를 런타임 중간(예: 메타데이터 파싱 단계)에 발견하면 암호 같은 에러 메시지가 출력되어 사용자가 문제를 진단하기 어렵습니다.
**Action:** 항상 스크립트 초반에 필수 외부 의존성의 존재 여부와 실행 권한을 명시적으로 검증(`[ ! -x "$CMD" ]`)하고, 누락 시 해결 가능한 설치 명령어(예: `brew install ffmpeg`)를 포함한 친절한 에러 메시지를 출력해야 합니다.
