...
## 2024-05-24 - 터미널 출력 파일 크기 표시 개선
**Learning:** 파일 크기를 단순히 바이트(bytes)로 표시하면 사용자가 큰 파일의 크기를 직관적으로 파악하기 어렵습니다. 스크립트 출력에서 크기를 KB, MB, GB와 같이 사람이 읽기 쉬운 형식으로 변환하여 표시하는 것이 훨씬 사용자 친화적(user-friendly)입니다.
**Action:** 터미널 출력 시 파일 크기나 메모리 양 등을 표시할 때는 바이트 단위 대신 사람이 읽기 쉬운 단위(KB, MB, GB)로 변환하는 포맷팅 로직(순수 bash의 산술 연산 등)을 적용해야 합니다.
