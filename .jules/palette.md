...
## 2026-07-17 - CLI 에러 메시지 시각적 분리
**Learning:** CLI 스크립트에서 에러 상태와 실행 가능한 해결책을 동일한 색상이나 평문으로 표시하면 인지 부하가 발생한다.
**Action:** 사용자에게 에러 원인은 빨간색(Red)으로, 실행 가능한 대안이나 해결책은 청록색(Cyan)으로 명확히 구분하여 제공하여 가독성을 높인다.
