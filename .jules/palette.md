...
## 2024-10-25 - 의존성 검증 누락으로 인한 난해한 에러 방지
**Learning:** CLI 스크립트에서 ffmpeg과 같이 널리 쓰이는 도구 외에도 ffprobe 등 동반되는 모든 외부 도구의 실행 가능 여부를 사전에 검증하지 않으면, 메타데이터 파싱 등 하위 작업에서 원인을 알 수 없는 난해한 에러가 발생합니다.
**Action:** 항상 스크립트 시작 시점에 사용되는 모든 외부 의존성(예: ffmpeg, ffprobe 둘 다)의 존재 및 실행 권한을 명시적으로 검증하여, 사용자에게 명확하고 조치 가능한 에러 메시지를 제공합니다.
