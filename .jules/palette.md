...
## 2023-10-25 - [CLI UX] 색상 구분을 통한 런타임 오류의 대안 행동(Fallback) 강조
**Learning:** 런타임 환경 문제(예: 접근 권한, 네트워크 차단 등)로 명령이 실패했을 때 사용자에게 제시되는 '대안(Fallback options)'이 단색의 텍스트 더미에 묻히면, 사용자가 문제 해결 방향을 놓치고 혼란을 겪는다는 것을 확인했습니다.
**Action:** 단순히 '설치 명령어'뿐만 아니라 논리적인 '대안 행동'을 제시할 때에도, 실행 가능한 조치나 권장 행동 부분에 눈에 띄는 색상(예: Cyan)을 적용하여 사용자가 무엇을 해야 하는지 직관적으로 인지할 수 있도록 시각적으로 분리해야 합니다.
