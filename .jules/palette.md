...
## 2026-08-12 - [CLI UX Enhancement] Human-Readable File Sizes
**Learning:** Displaying large numbers of bytes (e.g., `1048576 bytes`) creates unnecessary cognitive friction for the user. Translating raw numbers into familiar, human-readable formats (KiB, MiB, GiB) improves readability and immediately conveys the scale of downloaded files.
**Action:** Used POSIX `awk` to convert raw byte sizes into human-readable binary-prefixed sizes (KiB, MiB, GiB) in `download-reference.sh` output, ensuring cross-platform compatibility without relying on external non-standard utilities like `numfmt`.
