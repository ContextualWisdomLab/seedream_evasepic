...
## 2024-09-09 - Human-readable file sizes in CLI
**Learning:** Users find raw byte counts difficult to interpret at a glance, degrading the CLI experience. Standard utilities like `numfmt` are often missing in CI/CD environments.
**Action:** Implement human-readable file size formatting (KiB, MiB, etc.) using POSIX-compliant `awk` to ensure universal compatibility while significantly improving UX.
