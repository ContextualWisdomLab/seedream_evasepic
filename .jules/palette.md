...
## 2023-10-24 - [CLI 에러 메시지 시각적 분리]
**Learning:** CLI 환경에서 에러 상태와 해결 방법을 동일한 스타일로 표시하면 사용자의 인지 부하가 증가합니다.
**Action:** 에러 메시지는 붉은색(RED), 후속 조치나 해결 방법은 청록색(CYAN)으로 시각적으로 명확히 분리하여 사용자가 취해야 할 행동을 쉽게 식별하도록 합니다.
