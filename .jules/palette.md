...
## 2024-03-05 - 명시적인 의존성 누락 안내(ffprobe 검증)
**Learning:** CLI 도구에서 외부 명령어(ffprobe)의 의존성 누락이 명시적으로 처리되지 않으면, 예상치 못한 엉뚱한 하위 에러(예: '비디오 길이가 0보다 커야 합니다')가 발생하여 사용자가 근본 원인을 파악하기 어렵습니다.
**Action:** 모든 주요 외부 의존성(ffmpeg, ffprobe 등)은 스크립트 실행 초기에 개별적으로 검증하고, 누락 시 명확하고 실행 가능한 에러 메시지를 제공하여 사용자가 쉽게 문제를 인지하고 조치할 수 있도록 합니다.
