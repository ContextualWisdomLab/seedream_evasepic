...
## 2026-08-01 - 시각적으로 구분되는 예제 명령어(CLI UX)
**Learning:** CLI 사용법(Usage/Help) 및 오류 메시지에서 `Example:`로 제공하는 예제 명령어 전체(파일명 포함)를 다른 색상(예: Cyan)으로 시각적으로 눈에 띄게 강조하면, 사용자가 빠르게 복사 및 실행할 수 있어 인지 부하가 줄고 사용성이 크게 향상됨을 확인했습니다.
**Action:** 앞으로 모든 CLI 스크립트 작성 시 도움말 및 오류 안내에 포함되는 `Example:` 명령어는 반드시 ANSI 색상 코드를 적용해 시각적 하이라이팅을 적용할 것입니다.
