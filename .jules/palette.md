...
## 2025-03-01 - Explicit Dependency Validation for ffprobe
**Learning:** Missing explicit validation for all dependencies (like ffprobe, alongside ffmpeg) can lead to cryptic downstream failures during metadata parsing. Users need actionable and immediate error messages before the script attempts to run its main logic.
**Action:** Always explicitly validate the existence and executability of all external dependencies before executing the main logic.
