...
## 2025-02-28 - 의존성 검증 누락으로 인한 암묵적 실패 방지
**Learning:** `extract-frames.sh`와 같은 CLI 스크립트에서 메인 로직 실행 전 `ffprobe`와 같은 모든 외부 의존성의 존재 및 실행 가능 여부를 명시적으로 검증하지 않으면, 암묵적인 다운스트림 실패(예: 유효하지 않은 메타데이터 파싱)가 발생할 수 있습니다.
**Action:** 항상 스크립트 시작 부분에서 메인 로직이 의존하는 모든 외부 도구(예: `ffmpeg`, `ffprobe`)의 유효성을 검증하여 사용자에게 명확하고 실행 가능한 에러 메시지를 제공해야 합니다.
