...
## 2024-05-15 - 파일 크기 가독성 향상
**Learning:** 스크립트 실행 후 출력되는 단순 바이트 단위의 파일 크기는 사용자가 직관적으로 인식하기 어려우므로, MiB나 KiB 단위로 변환해 주는 것이 터미널 UX 개선에 효과적임을 배움.
**Action:** 앞으로 터미널 CLI 도구 개발 시 사용자가 접하는 파일 크기 등의 숫자는 항상 사람이 읽기 쉬운 포맷(human-readable)으로 변환하여 제공할 것.
