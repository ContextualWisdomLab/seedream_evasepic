...
## 2024-10-24 - CLI Output File Size Readability
**Learning:** Raw byte counts in CLI output are difficult for users to parse at a glance, increasing cognitive load. IEC units (KiB, MiB) are more precise and expected for binary file sizes than SI units (KB, MB).
**Action:** Always format raw byte sizes into human-readable IEC units in CLI tools.
