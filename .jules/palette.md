...
## 2026-08-10 - [CLI UX 개선] 의존성 자동 설치 후 PATH 누락 안내 추가
**Learning:** CLI 스크립트에서 누락된 도구(예: yt-dlp)를 자동 설치(pip 등)한 후에도 사용자 환경의 `$PATH` 설정 문제로 해당 도구를 실행하지 못하는 경우가 빈번히 발생함을 발견했습니다.
**Action:** 자동 설치 시도 직후에 다시 한번 `command -v`를 통해 도구의 실행 가능 여부를 검증하고, 접근 불가능할 경우 PATH 환경 변수 수정 방법(`export PATH="$HOME/.local/bin:$PATH"`)을 명시적으로 안내하는 에러 메시지를 출력하도록 UX를 개선합니다.
