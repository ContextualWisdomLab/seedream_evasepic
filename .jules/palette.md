...
## 2025-02-28 - Human Readable File Sizes
**Learning:** 원시 바이트(raw bytes) 단위로 파일 크기를 출력하면 사용자가 그 크기를 직관적으로 파악하기 어렵습니다. 특히 동영상 파일과 같이 큰 용량의 파일은 바이트 단위 출력이 인지 부하를 높입니다.
**Action:** 파일 크기를 출력할 때는 항상 사용자가 읽기 쉬운 형식(KiB, MiB, GiB 등 올바른 이진 접두사)으로 변환하여 제공하여 직관적인 UX를 제공하도록 합니다.
