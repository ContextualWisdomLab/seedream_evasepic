...
## 2026-08-11 - [CLI UX Enhancement] Explicit Missing Arguments Error and Precedence
**Learning:** Generic error messages like "Missing required argument(s)" decrease usability because users cannot immediately identify which parameter they forgot. Additionally, optional argument validation (e.g., model types) occurring before required argument checks causes confusing errors where a missing required argument is masked by an invalid default or optional argument.
**Action:** Replaced generic missing argument messages with targeted feedback specifying exactly which argument is missing (e.g., "<url>"). Moved required argument validation to precede any optional argument validation to ensure error reporting precedence.
