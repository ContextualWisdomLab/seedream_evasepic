...
## 2024-11-20 - Human-readable file size in CLI Output
**Learning:** Raw byte counts (e.g. `24581120 bytes`) in CLI outputs are not intuitive for end-users. Users prefer easily readable formats like `KB` or `MB` to quickly assess file sizes.
**Action:** Implemented a human-readable file size formatting logic in `download-reference.sh` to output sizes in MiB or KiB when appropriate, instead of always outputting raw bytes, resulting in a more user-friendly output.
