...
## 2024-09-04 - [CLI UX] 인간 친화적인 파일 크기 표시
**Learning:** CLI 도구에서 파일 크기(바이트)를 원시 숫자로 출력하면 사용자가 직관적으로 크기를 파악하기 어렵습니다. MB나 KB와 같은 인간 친화적인 단위를 병기(예: `14 MB (15403012 bytes)`)하면 UX가 크게 개선됩니다.
**Action:** 터미널에서 큰 숫자를 출력할 때는 항상 사용자가 한눈에 이해할 수 있는 단위(MB, KB 등) 포맷을 함께 제공하도록 합니다. Bash 산술 연산만으로 외부 프로세스 없이 빠르게 변환할 수 있습니다.
