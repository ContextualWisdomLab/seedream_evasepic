...
## 2024-05-18 - [CLI 에러 메시지와 사용법 안내를 위한 시각적 구분]
**Learning:** CLI 스크립트에서 에러 메시지(Red)와 사용법(Yellow), 그리고 예시(Cyan)는 이미 색상으로 잘 구분되어 있으나, 일부 사용자들은 에러 발생 시 출력되는 줄 간의 텍스트 여백이 없어 정보가 시각적으로 분산된다고 느낄 수 있음.
**Action:** 에러 메시지와 함께 빈 줄(newline)을 추가하여 시각적 여백(Whitespace)을 주면 에러를 더 명확히 인식하게 하고 읽기 편하게 만듦.
