...
## 2026-08-16 - [CLI UX Enhancement] 파일 크기를 Human-readable 포맷으로 표시
**Learning:** CLI 스크립트에서 파일 크기를 단순 바이트(raw byte count) 단위로 출력하면 사용자가 파일 크기를 직관적으로 인지하기 어렵습니다.
**Action:** 파일 크기 출력 시 `awk`를 사용하여 KB, MB, GB 등의 사람이 읽기 쉬운(human-readable) 포맷으로 변환하여 표시하도록 개선해야 합니다.
