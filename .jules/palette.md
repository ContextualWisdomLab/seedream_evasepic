...
## 2025-01-25 - 의존성 도구 실행 가능 여부 명시적 검증 (ffprobe)
**Learning:** bash 스크립트에서 ffmpeg 등 주 의존성은 검증하지만 ffprobe와 같이 함께 쓰이는 도구는 검증하지 않아, 이후 메타데이터 파싱 단계에서 사용자를 당황하게 만드는 이해하기 어려운 에러가 발생할 수 있음을 배웠습니다.
**Action:** 외부 의존성(예: ffmpeg, ffprobe 둘 다)은 스크립트 메인 로직이 실행되기 전에 모두 명확하게 실행 가능 여부를 검증하여 사용자에게 실행 가능한 에러 메시지를 제공합니다.
