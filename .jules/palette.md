...
## 2025-02-28 - 의존성 자동 설치 후 PATH 재검증 및 UX 개선
**Learning:** CLI 스크립트가 의존성(yt-dlp 등)을 자동 설치한 후, 해당 실행 파일이 시스템의 `$PATH`에 존재하지 않을 경우 다운스트림에서 알 수 없는 오류(예: command not found)가 발생할 수 있음을 확인했습니다.
**Action:** 의존성을 자동 설치한 직후 항상 `$PATH` 내 존재 여부를 재검증하고, 여전히 접근 불가할 경우 `$PATH` 문제 해결을 안내하는 실행 가능한 에러 메시지를 출력하도록 변경했습니다.
