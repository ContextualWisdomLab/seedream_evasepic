...
## 2023-10-25 - CLI 파일 크기 출력 가독성 향상
**Learning:** CLI 환경에서 다운로드한 파일의 크기를 바이트 단위로만 표시하면 사용자가 파일 크기를 직관적으로 인지하기 어렵습니다. KB, MB, GB 등의 사람이 읽기 쉬운 단위로 변환해 주면 UX가 크게 향상됩니다.
**Action:** `download-reference.sh`의 파일 크기 출력 부분에 awk를 사용하여 용량을 사람이 읽기 쉬운 형식(human-readable)으로 변환하도록 수정했습니다. 앞으로도 파일 크기 등 수치 정보를 출력할 때는 가독성을 고려할 것입니다.
