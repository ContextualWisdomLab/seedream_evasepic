...
## 2024-08-25 - Explicit CLI Error Messages
**Learning:** When building CLI tools, generic error messages like "Missing required argument(s)" provide a poor user experience. Users benefit significantly when the tool explicitly identifies exactly which argument is missing (e.g., `<url>`). Furthermore, required arguments must be validated before optional arguments to ensure errors are reported in a logical order, preventing confusing validation failures for optional parameters when the required ones are not even provided yet.
**Action:** Always validate required CLI arguments explicitly and report exactly which argument is missing. Enforce a strict validation order: required arguments first, then optional arguments.
