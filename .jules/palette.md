...
## 2026-07-20 - [CLI UX] 에러 상태와 해결 방안의 시각적 분리
**Learning:** 터미널 출력에서 에러 발생 원인과 해결 방안(Actionable steps)을 같은 색상(또는 기본 색상)으로 출력하면 사용자의 인지 부하가 증가함. 에러는 Red로, 해결 방안은 Cyan 등으로 시각적으로 분리하면 사용자가 다음에 해야 할 행동을 훨씬 직관적으로 파악할 수 있음.
**Action:** CLI 스크립트 작성 시 에러 메시지는 강렬한 색(Red)으로, 후속 조치나 해결 방안은 친숙한 색(Cyan 등)으로 분리하여 출력하도록 할 것.
