...
## 2024-05-18 - CLI 가독성 향상
**Learning:** 터미널과 같은 CLI 환경에서도 파일 크기(바이트 -> MB/GB)나 시간(초 -> 분:초)과 같은 정보는 사람이 읽기 쉬운 형태로 제공될 때 사용자 경험(UX)이 크게 향상된다.
**Action:** 숫자가 포함된 정보를 출력할 때, 단순 원시 데이터를 보여주기보다는 단위를 포함하여 사람이 한눈에 파악하기 쉽게 변환(human-readable formatting)하는 것을 기본으로 적용한다.
