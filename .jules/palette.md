...
## 2025-05-15 - [CLI UX Enhancement] Human-readable file sizes in download logs
**Learning:** CLI 스크립트에서 파일 크기를 바이트 단위로만 출력하면 사용자가 파일의 실제 크기를 직관적으로 파악하기 어렵습니다 (예: "12345678 bytes" 대신 "11.77 MiB").
**Action:** `download-reference.sh` 스크립트의 파일 크기 출력 부분에 awk를 활용하여 KiB, MiB, GiB 단위로 변환해주는 로직을 추가하여 터미널 내 가독성과 정보 인지 속도를 개선했습니다.
