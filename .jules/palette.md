...
## 2026-07-18 - 명시적인 의존성 검증을 통한 UX 개선
**Learning:** ffmpeg와 ffprobe처럼 여러 외부 도구에 의존하는 스크립트의 경우, 종종 주 도구만 검증하여 부 도구가 누락되었을 때 알기 어려운 후속 오류를 유발합니다. 모든 의존성을 명시적으로 검증하면 오류 명확성이 향상된다는 점을 배웠습니다.
**Action:** 메인 로직을 실행하기 전에 항상 모든 외부 의존성(예: ffmpeg 및 ffprobe 모두)의 존재 및 실행 가능 여부를 명시적으로 검증하여, 사용자에게 명확하고 조치 가능한 오류 메시지를 제공할 것입니다.
