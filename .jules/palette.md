...
## 2024-10-24 - 터미널 파일 크기 표시 단위 개선
**Learning:** 터미널 CLI 환경에서 raw bytes 단위로 파일 크기를 출력하는 것은 사용자가 크기를 한눈에 인지하기 어렵게 만들며 UX(사용자 경험)를 저하시킵니다. 표준 POSIX awk를 사용해 KiB, MiB 단위로 변환해 보여주는 것이 호환성과 UX 측면에서 훨씬 좋습니다.
**Action:** 향후 스크립트 작성 시 사용자가 확인해야 하는 파일 크기는 가독성을 고려해 사람이 읽기 쉬운(Human-readable) 포맷과 단위(KiB, MiB, GiB)를 사용하여 출력하도록 구현해야 합니다.
