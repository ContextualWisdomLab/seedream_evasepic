...
## 2024-05-16 - 터미널 파일 크기 표시 가독성 개선
**Learning:** CLI 환경에서 단순 바이트(byte) 단위 출력은 사용자가 파일 크기를 직관적으로 파악하기 어렵게 만듭니다. 사용자는 1234567 bytes 대신 1.18 MiB 와 같은 익숙한 단위를 선호합니다.
**Action:** POSIX 호환 awk를 활용하여 추가 종속성 없이 KiB, MiB 등의 사람이 읽기 쉬운 단위로 파일 크기를 포맷팅하도록 개선했습니다.
