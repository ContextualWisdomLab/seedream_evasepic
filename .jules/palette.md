...
## 2025-02-28 - [CLI UX Enhancement] 파일 크기의 인간 친화적 표기 (Human-Readable Size)
**Learning:** CLI 스크립트에서 파일 크기를 원시 바이트(byte) 단위로 출력하면 사용자가 직관적으로 가늠하기 어렵습니다. 읽기 쉬운 형태(KB, MB, GB 등)로 변환하여 제공하면 가독성과 사용자 경험(UX)이 향상됩니다.
**Action:** 파일 크기를 출력할 때는 `awk` 등을 활용해 읽기 쉬운 형식(human-readable format)으로 변환하여 출력하도록 개선했습니다.
