...
## 2024-07-28 - 에러 상태와 해결 방법의 시각적 분리
**Learning:** 에러 메시지와 해결 방법을 같은 색상이나 기본 텍스트 색상으로 제공하면 사용자의 인지 부하가 가중됨.
**Action:** 앞으로 CLI 스크립트에서 에러 원인은 Red로, 사용자가 취할 수 있는 해결 방법(Fallback options 등)은 Cyan으로 출력하여 시각적으로 분리함.
