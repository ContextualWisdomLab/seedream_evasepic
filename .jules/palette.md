...
## 2025-05-15 - [CLI UX Enhancement] Human Readable File Size
**Learning:** CLI 사용자가 다운로드된 파일 크기를 바이트 단위로만 보게 되면 직관적으로 얼마나 큰 파일인지 알기 어렵습니다. 특히 영상 파일의 경우 MB 단위로 출력하는 것이 훨씬 더 가독성이 좋고 인지 부하를 줄일 수 있습니다.
**Action:** `download-reference.sh` 스크립트가 파일 크기를 출력할 때, 단순 바이트 단위가 아닌 B, KB, MB, GB 등으로 사람이 읽기 쉬운 포맷(Human Readable)으로 변환하여 출력하도록 개선하였습니다.