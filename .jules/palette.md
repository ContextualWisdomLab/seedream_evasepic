...
## 2024-07-24 - ffprobe 의존성 검증 추가
**Learning:** CLI 스크립트에서 외부 의존성(예: ffprobe)의 존재 여부를 명시적으로 확인하지 않으면 알 수 없는 하위 오류(메타데이터 파싱 실패 등)가 발생할 수 있습니다.
**Action:** 메인 로직을 실행하기 전에 모든 외부 의존성(ffmpeg 및 ffprobe 둘 다)의 존재와 실행 가능 여부를 항상 명시적으로 검증하여 사용자에게 명확하고 조치 가능한 오류 메시지를 제공합니다.
