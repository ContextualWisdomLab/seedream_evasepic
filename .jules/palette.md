...
## 2026-07-28 - 명시적인 의존성 검증 추가
**Learning:** 주요 의존성(예: ffmpeg)만 검증하고 부가적인 도구(예: ffprobe)의 검증을 누락하면, 후속 로직에서 난해한 오류가 발생하여 사용자가 문제의 원인을 파악하기 어렵다는 것을 확인했습니다.
**Action:** 스크립트 실행 전, 본 로직에서 사용되는 모든 외부 의존성의 존재 여부와 실행 권한을 명시적으로 검증하여 사용자에게 명확하고 실천 가능한 에러 메시지를 제공합니다.
