...
## 2024-11-20 - [CLI UX Enhancement] 파일 크기를 사람이 읽기 쉽게 출력
**Learning:** CLI 스크립트에서 파일 크기를 바이트 단위로만 출력하면 사용자가 직관적으로 크기를 체감하기 어렵다는 것을 배웠습니다.
**Action:** 앞으로는 파일 크기를 출력할 때 항상 사람이 읽기 쉬운 형식(KB, MB, GB)으로 변환하여 출력하도록 해야 합니다.
## 2024-11-20 - [CLI UX Enhancement] 스크립트 상대 경로 참조
**Learning:** 테스트 스크립트 등에서 다른 스크립트를 호출할 때 `./`와 같이 현재 작업 디렉토리에 의존하는 상대 경로를 사용하면, CI 환경이나 다른 디렉토리에서 실행될 때 파일 참조 오류가 발생하여 테스트가 취약해짐을 배웠습니다.
**Action:** 스크립트 실행 경로를 참조할 때는 항상 `$(dirname "$0")`나 스크립트 내에서 정의한 기준 디렉토리 변수(`$SCRIPT_DIR`)를 사용하여 명확하게 경로를 지정해야 합니다.
