...
## 2025-02-28 - 명확한 누락 인자 오류 안내 추가
**Learning:** 사용자가 스크립트 실행 시 어떤 인자가 누락되었는지 포괄적인 "Missing required argument(s)" 오류로 안내하면, 해결 방법(어떤 인자를 추가해야 하는지)을 파악하기 어렵다는 점을 배웠습니다.
**Action:** 필수 인자가 누락된 경우, 구체적으로 어떤 인자(예: `<url>`, `<output_dir>`)가 누락되었는지 명시하는 구체적인 에러 메시지를 제공하여 사용자가 더 빠르게 수정하고 재시도할 수 있도록 개선합니다.
