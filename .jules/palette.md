...
## 2025-02-28 - 명확한 CLI 오류 메시지 제공 및 검증 순서 개선
**Learning:** CLI 도구에서 'Missing required argument(s)'와 같은 포괄적인 에러 메시지보다 'Missing required argument: <url>'처럼 어떤 인자가 누락되었는지 명확히 짚어주는 것이 UX 측면에서 사용자 혼란을 줄이고 가이드를 제공하는데 훨씬 유리하다는 것을 확인했습니다. 또한, 선택적 인자(optional argument)를 평가하기 전에 필수 인자 누락 여부를 먼저 검증해야 사용자에게 올바른 에러 리포팅 우선순위를 제공할 수 있습니다.
**Action:** 향후 CLI 스크립트 작성 시 필수 인자 검증 로직을 항상 최우선으로 배치하고, 각 인자별로 구체적인 누락 항목을 명시하는 에러 메시지를 작성하도록 합니다.
