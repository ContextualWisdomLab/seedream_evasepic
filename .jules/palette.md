...
## 2024-03-24 - [CLI UX Enhancement] Human-readable File Sizes
**Learning:** 터미널 출력에서 바이트 단위의 큰 숫자를 그대로 노출하면 사용자가 크기를 직관적으로 파악하기 어렵습니다. 인지 부하를 줄이기 위해 파일 크기를 KB, MB, GB 등의 읽기 쉬운 형식으로 변환하여 표시하는 UX 개선을 진행했습니다.
**Action:** `download-reference.sh` 스크립트에서 파일 크기 출력 시 `awk`를 사용하여 사람이 읽기 쉬운 단위(B, KB, MB, GB)로 변환하도록 개선했습니다.
