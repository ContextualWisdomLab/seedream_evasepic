...
## 2025-02-28 - Explicit Dependency Validation
**Learning:** UX in CLI scripts requires explicit validation of all external dependencies (like `ffprobe`) prior to execution, rather than relying solely on main tool checks (like `ffmpeg`). Missing this causes cryptic downstream errors.
**Action:** Always explicitly validate the existence and executability of all required external CLI dependencies before executing the main logic, and provide actionable error messages (e.g., using CYAN for installation instructions).
