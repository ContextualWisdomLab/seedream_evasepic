...
## 2024-05-24 - CLI Missing Argument Error Clarity
**Learning:** Users often struggle with generic "Missing required argument(s)" errors in CLI tools because they don't explicitly identify which parameter is at fault, leading to friction in correctly formatting the command. Furthermore, validating optional arguments before required ones can result in confusing error precedence (e.g., complaining about an invalid optional model before checking if the required input file was even provided).
**Action:** Always validate required arguments first. Replace generic error messages with targeted feedback (e.g., "Missing required argument: <url>") to immediately guide the user to the correct fix.
