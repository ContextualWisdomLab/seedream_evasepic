...
## 2026-09-23 - [파일 크기 출력 개선]
**Learning:** 터미널 출력에서 바이트(bytes) 단위의 파일 크기는 사용자가 직관적으로 이해하기 어렵습니다. 특히 동영상 파일의 경우 MB 단위로 변환해 제공하는 것이 인지적 부하를 줄여줍니다.
**Action:** 터미널 출력 시 파일 크기를 자동으로 MB 또는 KB로 변환하여 출력하는 로직을 추가했습니다. 추후 CLI 스크립트 작성 시 사용자 친화적인 단위 포맷팅을 우선적으로 고려해야 합니다.
