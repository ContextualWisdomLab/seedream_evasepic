...
## 2024-05-30 - CLI 출력에서 사람이 읽기 쉬운 파일 크기 표시
**Learning:** 터미널 기반 CLI UX는 리소스 소비에 대한 명확하고 즉각적인 피드백이 필요합니다. 비디오 다운로드의 경우(기가바이트에 달할 수 있음) 바이트 단위로 표시하면 사용자가 한눈에 파악하기 어렵고 도구의 완성도가 떨어져 보입니다.
**Action:** 외부 비표준 유틸리티(numfmt 등)에 의존하지 않고 크로스 플랫폼 호환성을 위해 표준 POSIX awk를 사용하여 파일 크기 등 큰 숫자 값을 사람이 읽기 쉬운 단위(KiB, MiB, GiB)로 변환하여 출력합니다. 이를 통해 CLI 출력을 확인하는 사용자에게 더 직관적이고 고급스러운 경험을 제공합니다.
