...
## 2026-08-12 - [CLI UX Enhancement] Human-readable file size output
**Learning:** CLI 출력에서 비디오나 고해상도 이미지와 같은 큰 미디어 파일의 원시 바이트 수는 한눈에 파악하기 어렵습니다. 파일 크기를 MB나 KB 단위로 변환하여 표시하면 가독성과 사용자의 이해도를 크게 향상시킬 수 있음을 확인했습니다.
**Action:** `download-reference.sh` 스크립트에서 파일 크기를 바이트 대신 MB나 KB 단위로 동적 포맷팅하여 출력하도록 개선했으며, 향후 파일 크기를 보고하는 다른 스크립트에도 이 패턴을 적용할 예정입니다.
