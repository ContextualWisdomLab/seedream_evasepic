...
## 2026-07-19 - [CLI 오류 메시지와 해결책 분리]
**Learning:** 터미널 출력 시 오류 메시지와 해결 방법을 단순한 텍스트로 제공할 경우 사용자의 인지 부하가 증가함. Red/Cyan 색상 대비를 사용하여 에러 상태(Red)와 실행 가능한 해결책(Cyan)을 시각적으로 분리하면 즉각적인 가독성 및 사용성이 향상됨.
**Action:** bash 스크립트 작성 시 에러 원인은 Red로, 이후 사용자에게 제안하는 조치사항 및 대안은 항상 Cyan으로 분리하여 출력하도록 적용함.
