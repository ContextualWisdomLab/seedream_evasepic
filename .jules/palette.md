...
## 2025-03-01 - [CLI UX Enhancement] 의존성 검증 강화
**Learning:** CLI 스크립트에서 외부 도구(예: `ffmpeg`, `ffprobe` 등)를 사용할 때 메인 로직 실행 전에 모든 의존성의 존재 여부를 명시적으로 확인하지 않으면, 스크립트 실행 중 알 수 없는 에러나 빈 결과값이 반환되어 사용자가 문제를 파악하기 매우 어렵다는 점을 배웠습니다.
**Action:** 외부 의존성을 사용하는 스크립트 작성 시, 단순히 하나의 도구만 확인하지 않고 사용되는 모든 도구(예: `ffprobe` 포함)에 대해 명시적이고 실행 가능한 에러 메시지와 함께 의존성 검증 로직을 추가합니다.
