...
## 2025-02-28 - [CLI UX] 명시적인 외부 의존성 검증
**Learning:** 스크립 실행 중 하위 프로세스(예: ffprobe를 통한 메타데이터 파싱)가 암묵적으로 실패하게 되면 원인을 알 수 없는 난해한 에러가 발생하여 사용자 경험이 크게 저하된다는 점을 배웠습니다.
**Action:** 메인 로직을 실행하기 전에 ffmpeg뿐만 아니라 ffprobe와 같은 모든 외부 의존성의 존재 및 실행 가능 여부를 명시적으로 검증하여 사용자에게 명확하고 조치 가능한 에러 메시지를 제공합니다.
