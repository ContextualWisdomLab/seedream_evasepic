...
## 2025-05-15 - 파일 크기 가독성 개선 (Human-Readable Sizes)
**Learning:** CLI 스크립트에서 파일 크기 등을 원시 바이트(raw byte) 단위로 출력하면 사용자가 크기를 한눈에 체감하기 어렵습니다. 읽기 편한 형식(KB, MB, GB)으로 변환하여 제공하면 터미널 환경에서의 UX가 크게 향상됩니다.
**Action:** 파일 크기를 계산하여 출력할 때 항상 `awk` 등의 도구를 사용하여 사람이 읽기 쉬운 형식으로 변환한 뒤 출력합니다.
