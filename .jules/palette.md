...
## 2023-11-20 - 의존성 검증 강화 (ffprobe 추가)
**Learning:** 주요 로직 실행 전 모든 외부 의존성(예: ffmpeg 및 ffprobe)을 검증하지 않으면 의존성이 없을 때 파싱 에러와 같은 알아보기 힘든 하위 오류가 발생할 수 있습니다.
**Action:** 항상 모든 의존성 도구의 존재와 실행 가능성을 명시적으로 검증하여 사용자에게 명확하고 조치 가능한 에러 메시지를 제공합니다.
