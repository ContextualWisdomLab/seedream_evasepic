...
**Learning:** bash 스크립트에서 외부 입력값을 명령어 인자로 넘길 때 하이픈(-)으로 시작하는 문자열이 옵션으로 오인될 수 있음
...
## 2026-10-24 - Prevent symlink TOCTOU attacks via staging directories
**Vulnerability:** Checking `[ -L "$OUTPUT" ]` before passing the path to `yt-dlp` leaves a Time-of-Check to Time-of-Use (TOCTOU) race condition. An attacker can replace the target path with a symlink after the check but before `yt-dlp` opens the file, leading to arbitrary file overwrite.
**Learning:** Shell-level pre-checks on pathnames cannot guarantee identity at the time of use. Robust defense against symlink attacks requires native file descriptor semantics or private staging environments.
**Prevention:** Download untrusted network payloads into a private, same-filesystem staging directory (e.g., `mktemp -d`), then use `mv` to publish the completed artifact. The `mv` command atomically replaces the target directory entry (even if it became a symlink) without following it, neutralizing the race condition.
