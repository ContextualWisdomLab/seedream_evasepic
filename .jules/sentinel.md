...
## 2026-09-14 - ffprobe metadata trust-boundary validation
**Finding:** `extract-frames.sh` neutralized `ffprobe`-derived duration, resolution, and FPS values before terminal rendering, but accepted and persisted those fields without first checking the structural contract expected from `ffprobe`. The selected fields are numeric or rational metadata; arbitrary external-tool output should not silently become `metadata.txt` data or reach downstream arithmetic. The prior `[CRITICAL]` wording overstated the evidence because no ordinary-media exploit path was demonstrated for these typed fields.
**Learning:** Terminal neutralization and input validation solve different problems. Output encoding keeps control bytes inert at a terminal sink, while structural validation prevents malformed external-command output from entering the application state in the first place.
**Prevention:** Validate duration as a non-negative decimal, width/height as positive integers when present, and FPS as a non-negative rational with a positive denominator before persisting or rendering them. Keep terminal neutralization as defense in depth, and exercise the boundary with a stubbed `ffprobe` that returns malformed control-bearing metadata.
