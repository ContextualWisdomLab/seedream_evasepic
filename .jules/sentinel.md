...
## 2024-07-18 - Terminal Output Injection via caller-controlled path
**Vulnerability:** A status log `printf "%b\n" "${GREEN}File already exists, skipping download: ${NC}$OUTPUT"` interpolated the caller-controlled output path (`$OUTPUT`) through `%b`, which evaluates escape sequences.
**Learning:** Bypassing terminal-neutralization helpers or using `%b` on variables containing file paths allows attackers to embed malicious ANSI escape sequences (Terminal Output Injection) if they control the filename.
**Prevention:** Always use `%s` instead of `%b` when printing dynamically derived or user-controlled variables in `printf` (e.g., `printf "%b%s\n" "${COLOR}" "$USER_INPUT"`).
