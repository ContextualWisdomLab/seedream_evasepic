...
## 2026-08-14 - printf %b 포맷 취약점 패치 (터미널 제어 문자열 주입)
**Vulnerability:** `terminal_print_value` 함수에서 `printf`를 사용할 때 `%b%s%b` 포맷을 사용하면, 세 번째 인자인 `suffix`에 대해 백슬래시 이스케이프가 해석되어 의도치 않은 제어 문자가 주입되거나 터미널 디스플레이가 깨지는 등 보안 취약점이 발생할 수 있습니다.
**Learning:** 터미널 출력 포맷에서 사용자 입력값이나 무력화된 값이 포함될 수 있는 부분은 절대로 `%b` 포맷 지정자를 사용하여 출력해선 안 되며 리터럴 출력을 위해 반드시 `%s`를 써야 합니다.
**Action:** `terminal_print_value` 함수의 `printf` 출력 포맷을 `%b%s%b\n`에서 `%b%s%s\n`으로 수정하여 세 번째 인자에 대해서도 이스케이프 시퀀스가 평가되지 않고 안전한 텍스트로만 출력되게 하였습니다.
