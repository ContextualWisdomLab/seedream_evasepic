...
## 2026-08-28 - [CRITICAL] Fix terminal injection via media metadata
**Vulnerability:** The `ffprobe` output for duration, resolution, and fps was printed directly using `printf "%b"` in `extract-frames.sh`. Since this metadata can be arbitrarily crafted in a malicious media file, it could contain ANSI escape sequences to perform terminal injection attacks.
**Learning:** Terminal injection attacks aren't limited to user-provided paths or explicitly typed inputs. Any metadata extracted from user-supplied files (like media files) using external tools (like `ffprobe`) must be treated as untrusted input.
**Prevention:** Always sanitize extracted metadata using `terminal_safe_text` and output it safely using `%s` before combining it with terminal control sequences. Update static analysis checks to catch unsanitized metadata outputs.
