...
## 2026-08-31 - Symlink Override / TOCTOU Vulnerability Fix in Cache Validation
**Vulnerability:** A cache-hit condition `[ -f "$OUTPUT" ] && [ -s "$OUTPUT" ]` failed to reject symbolic links on a target output path. If an attacker pre-created a symlink at the target location pointing to a sensitive file (e.g., `/etc/passwd`), the subsequent application logic (e.g., downloading a video via `yt-dlp`) would follow the symlink and overwrite the target, rather than rejecting the symlink.
**Learning:** Symlink resolution (following the link to its destination) occurs silently in many shell utilities (`[ -f ]`, `[ -s ]`, `echo >`). A standard `-f` existence check does not block a symlink.
**Prevention:** Always insert an explicit symlink rejection (`if [ -L "$OUTPUT" ]; then exit 1; fi`) immediately before validating cache conditions or initiating writes on any output path. Apply this strictly to target output paths (not necessarily input paths that are safely read).
