...
## 2026-10-24 - Static terminal-output contract missing variables
**Vulnerability:** While `DURATION`, `RESOLUTION`, and `FPS` were previously converted to be handled via `terminal_safe_text` inside `extract-frames.sh`, they were not appended to the static grep regex that enforces the security contract in `test_terminal_output.sh`. This left the variables unguarded from regression since `printf "%b"` might be introduced again for these.
**Learning:** When adding or fixing output values that require terminal neutralization, the static test enforcement list (e.g. `\$DURATION|\$RESOLUTION`) must also be updated to ensure the security boundary does not regress.
**Prevention:** Update `test_terminal_output.sh`'s `grep` command to explicitly include `\$FRAME_COUNT|\$DURATION|\$RESOLUTION|\$FPS` so that any future unescaped usage is caught by tests.
