...
## 2024-09-25 - [CRITICAL] 경로 조작 방지를 위한 파일 경로 인자 검증 및 접두사 추가
**Vulnerability:** `ffmpeg`, `ffprobe`, `whisper` 명령어에 사용자 제어 파일 경로를 전달할 때 `--`를 사용하면 의도치 않은 인자 주입(Argument Injection) 취약점이 발생합니다.
**Learning:** `--`는 `ffmpeg` 등에서 옵션의 끝을 나타내는 구분자로 지원되지 않으며, 사용자 입력이 실제 옵션이나 출력 대상으로 잘못 해석될 수 있습니다.
**Prevention:** `--`를 사용하는 대신, 경로 검증을 수행하고 상대 경로가 하이픈으로 시작하는 경우 `./`를 접두사로 추가해야 합니다 (예: `[[ "$FILE" != /* ]] && [[ "$FILE" == -* ]] && FILE="./$FILE"`).
