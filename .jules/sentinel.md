...
## 2026-08-18 - [CRITICAL] Prevent Arbitrary File Overwrite via Symlink in cache check
**Vulnerability:** In `download-reference.sh`, checking for cache existence with `[ -f ]` and `[ -s ]` does not safely handle symlinks. An attacker could provide a symlink pointing to a sensitive file, causing it to be overwritten or manipulated.
**Learning:** Attempting to mitigate this by simply appending `&& [ ! -L ]` to the `if` condition fails because it treats symlinks as a cache miss, allowing execution to proceed and overwrite the target file.
**Prevention:** Explicitly check `[ -L ]` and abort immediately (`exit 1`) *before* performing any cache or existence checks on output paths.
