...
## 2026-09-22 - [CRITICAL] ffprobe, ffmpeg, whisper 명령어 옵션 주입 취약점 방지
**Vulnerability:** ffmpeg, ffprobe, whisper 등의 도구는 사용자 파일 경로 앞에 `--` (end-of-options) 구분자를 안정적으로 지원하지 않거나, 이 구분자를 실제 입력 파일명으로 오인하여 옵션 주입 취약점을 유발할 수 있습니다.
**Learning:** `--` 를 지원하지 않는 도구의 경우, 사용자 통제 하에 있는 파일 경로가 하이픈(-)으로 시작할 때 명령어 옵션으로 해석될 위험이 큽니다.
**Prevention:** `--` 를 사용하지 않는 대신, 경로를 정규화하여 절대 경로가 아니면서 하이픈으로 시작하는 경우 파일 경로 앞에 `./` 를 명시적으로 추가(`[[ "$FILE" != /* ]] && [[ "$FILE" == -* ]] && FILE="./$FILE"`)하여 옵션 주입을 방지해야 합니다.
