...
## 2026-08-16 - ffprobe metadata ANSI injection
**Vulnerability:** Untrusted ffprobe metadata ($DURATION, $RESOLUTION, $FPS) was printed directly using `printf "%b"` without sanitization, leading to potential ANSI escape sequence injection.
**Learning:** External tool output (like ffprobe/ffmpeg) must always be treated as untrusted user input, even if it seems like internal metadata.
**Prevention:** Always use a terminal escape neutralizer (e.g., `terminal_safe_text`) on all metadata extracted from media files before outputting them to a terminal.
