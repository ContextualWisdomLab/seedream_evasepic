...
## 2026-08-06 - ANSI Escape Sequence Injection via ffprobe metadata
**Vulnerability:** External metadata extracted from media files via `ffprobe` (like `DURATION`, `RESOLUTION`, `FPS`) was printed to the terminal using `printf "%b"` without neutralization, which could allow ANSI escape sequence injection if a malicious media file was crafted with forged metadata.
**Learning:** Metadata extracted from media files via external tools must be treated as untrusted user input. A terminal renderer must neutralize these variables.
**Prevention:** All untrusted input variables printed to the terminal, including metadata from files, must be neutralized using `terminal_safe_text` and output with `%s` instead of `%b`. Added regex to check for these variables in `test_terminal_output.sh`.
