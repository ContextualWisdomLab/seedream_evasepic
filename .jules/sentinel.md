...
## 2026-09-07 - Neutralize untrusted metadata variables in terminal output
**Vulnerability:** Untrusted variables extracted from video metadata (`$DURATION`, `$RESOLUTION`, `$FPS`) were formatted into `printf "%b"` directly, allowing ANSI escape sequence injection to manipulate the terminal display.
**Learning:** External variables sourced from files (like `ffprobe` output) must be treated as untrusted and neutralized before displaying in the terminal. The static `printf "%b"` contract should include these variables.
**Prevention:** Always wrap untrusted variables in a neutralization function like `terminal_safe_text` and output using `%s` in the `printf` format instead of interpolating into `%b`. Enforce this across all dynamic variables with a regression test.
