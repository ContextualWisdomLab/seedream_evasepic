...
## 2024-08-15 - [CRITICAL] ANSI 인젝션 취약점 수정 (ffprobe 메타데이터)
**Vulnerability:** `extract-frames.sh`에서 `ffprobe`를 통해 추출한 메타데이터(`$DURATION`, `$RESOLUTION`, `$FPS`)를 검증 없이 `printf "%b"`에 직접 포함시켜 ANSI 이스케이프 시퀀스 인젝션 취약점이 존재했습니다.
**Learning:** 외부 도구(예: `ffprobe`)의 출력값은 항상 신뢰할 수 없는 사용자의 입력으로 간주해야 하며, 터미널에 출력할 때 악의적인 제어 문자가 실행될 수 있음을 배웠습니다.
**Prevention:** `printf`를 사용할 때 `%b`는 신뢰할 수 있는 ANSI 색상 코드에만 사용하고, 신뢰할 수 없는 데이터는 반드시 `%s`를 사용하여 출력해야 합니다. 또한 정적 분석 스크립트에 해당 변수들을 추가하여 회귀를 방지해야 합니다.
