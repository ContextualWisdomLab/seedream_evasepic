...
## 2026-08-29 - TOCTOU Symlink Arbitrary File Overwrite
**Vulnerability:** Shell scripts in analyze-reference-video checked `[ -f "$OUTPUT" ]` for a cache hit but did not explicitly reject symlinks (`[ -L "$OUTPUT" ]`) before executing external tools like yt-dlp or ffmpeg, allowing an attacker to overwrite arbitrary files by pointing the output path to a sensitive file.
**Learning:** Shell test conditions like `-f` return true for symlinks if the target file exists. Relying on this for early exit or assuming external tools will safely handle symlinks creates a Time-of-Check to Time-of-Use (TOCTOU) vulnerability where symlink resolution can be maliciously weaponized.
**Prevention:** Always strictly reject symlinks immediately before evaluating cache conditions or executing external commands that write to user-controlled paths. Do not assume tools like yt-dlp, ffmpeg, or whisper are safe against symlink attacks.
