...
## 2026-08-28 - Symlink TOCTOU in Output Paths
**Vulnerability:** The script blindly wrote to a target output path without first checking if the path was a symlink.
**Learning:** This introduces a Time-of-Check to Time-of-Use (TOCTOU) vulnerability where an attacker could replace the output path with a symlink to an arbitrary system file, causing the script to overwrite it with downloaded data.
**Prevention:** Always explicitly check for and reject symlinks (e.g., `[ -L "$OUTPUT" ]`) before evaluating cache conditions or writing to user-provided file paths.
