...
## 2023-10-24 - [터미널 출력 스푸핑 취약점 수정]
**Vulnerability:** `extract-frames.sh` 스크립트는 `ffprobe`를 이용해 추출한 메타데이터(`DURATION`, `RESOLUTION`, `FPS`)를 사용자 제어 변수로 간주하지 않고 `printf "%b"`를 통해 그대로 출력했습니다.
**Learning:** 비디오 파일의 메타데이터 속성 역시 조작될 수 있는 사용자 입력입니다. `%b`를 통해 출력하면 악의적인 이스케이프 시퀀스나 터미널 제어 문자가 실행될 수 있습니다.
**Prevention:** 신뢰할 수 없는 모든 출력은 터미널에 렌더링하기 전에 항상 `terminal_safe_text`로 무효화하고 `%b`가 아닌 `%s`를 사용하여 출력해야 합니다.
