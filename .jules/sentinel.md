...
## 2025-02-18 - [터미널 제어 문자 삽입 취약점 수정 (ffprobe 메타데이터)]
**Vulnerability:** ffprobe를 통해 추출된 비디오 메타데이터(`$DURATION`, `$RESOLUTION`, `$FPS`)가 터미널에 직접 출력되는 취약점을 발견했습니다. 악의적으로 조작된 메타데이터를 포함한 비디오 파일은 터미널에서 C0/C1 제어 문자를 실행하도록 허용하여 스크립트 실행 환경을 오염시키고 시스템 정보를 유출할 위험이 있습니다.
**Learning:** 외부 바이너리(`ffprobe`)를 통해 파싱된 데이터라 할지라도, 사용자가 제어할 수 있는 입력 파일(비디오)에서 파생된 값은 항상 신뢰할 수 없는 데이터로 취급해야 한다는 점을 배웠습니다.
**Prevention:** 사용자 제어 입력값과 파생된 메타데이터를 터미널에 출력하기 전에는 항상 `terminal_safe_text`와 같은 정화 함수를 사용하여 제어 문자를 무효화하고, `test_terminal_output.sh`와 같은 정적 분석 테스트에 출력 변수를 등록하여 안전한 출력을 보장해야 합니다.
