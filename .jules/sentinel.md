...
## 2026-09-03 - Prevent Command Injection via ANSI Escapes in `ffprobe` output
**Vulnerability:** In `plugins/seedream-evasepic/skills/analyze-reference-video/scripts/extract-frames.sh`, variables generated from untrusted user input parsed from `ffprobe` (like `$DURATION`, `$RESOLUTION`, and `$FPS`) were being printed using `printf "%b\n"`. This exposes the terminal to ANSI escape sequence injection if a malicious video file is provided, as these strings are interpreted and can execute arbitrary terminal controls.
**Learning:** Metadata extracted from media files via external tools like `ffprobe` (e.g., duration, resolution, fps) must be treated as untrusted user input. Using `%b` in `printf` with these values allows for ANSI escape sequence injection attacks.
**Prevention:** Always sanitize such values before outputting them to a terminal using a terminal escape neutralizer (e.g., `terminal_safe_text`). Additionally, when printing neutralized data alongside trusted ANSI colors, you must use `%s` for the neutralized string (e.g., `printf '%b%s' "$COLOR" "$SAFE_VAR"`); passing neutralized text through `%b` is dangerous because it will re-evaluate the safe `\x` hex representations back into live control bytes.
