...
## 2026-08-06 - Prevent ANSI Escape Sequence Injection from external media metadata
**Vulnerability:** External tools like `ffprobe` can return metadata (e.g. `$DURATION`, `$RESOLUTION`, `$FPS`) containing ANSI escape sequences or control characters if the media file is maliciously crafted. Directly interpolating these values into `printf "%b"` can lead to terminal spoofing/injection.
**Learning:** Metadata extracted from media files must be treated as untrusted user input, just like filenames or URLs. Furthermore, to avoid subshell performance overhead, terminal neutralization functions should support `printf -v` to assign output directly to variables.
**Prevention:** Route all untrusted metadata through `terminal_safe_text` and output them using `%s` alongside trusted `%b` ANSI colors in `printf`.
