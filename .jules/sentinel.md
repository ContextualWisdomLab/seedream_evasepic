...
## 2026-08-20 - [CRITICAL] Untrusted Metadata ANSI Injection in `extract-frames.sh`
**Vulnerability:** Metadata values extracted from media files via `ffprobe` (such as duration, resolution, and fps) were implicitly trusted and passed directly into `printf "%b"`. If a malicious file contained ANSI escape sequences in its metadata, it could lead to ANSI escape sequence injection attacks in the terminal.
**Learning:** Metadata extracted from media files using external tools must be treated as untrusted user input, just like paths or URLs.
**Prevention:** Sanitize metadata values using a terminal escape neutralizer (e.g., `terminal_safe_text`) before printing them to the terminal. Use `%s` for neutralized strings alongside trusted ANSI colors formatted with `%b`. Furthermore, update static analysis regexes in test scripts to enforce this for any newly processed metadata variables.
