...
## 2024-09-10 - Terminal Control Injection via Metadata Variables
**Vulnerability:** Untrusted variables parsed from video metadata (`$DURATION`, `$RESOLUTION`, `$FPS` via `ffprobe`) were interpolated directly into a `printf "%b"` output string. This could allow an attacker to inject ANSI escape sequences or control characters by crafting a malicious media file.
**Learning:** Even internal tool outputs (like `ffprobe`) must be considered untrusted input when they reflect data embedded in attacker-controlled files.
**Prevention:** All terminal output containing parsed variables must be routed through `terminal_safe_text` or strictly type-validated (e.g. integer checks) prior to rendering.
