...
## 2026-09-01 - Prevent Cache Hit Symlink TOCTOU Attack
**Vulnerability:** Cache hit check relied only on `-f` and `-s` flags, which follows symlinks. This meant a malicious symlink at the output path pointing to a sensitive file could be exploited, leading to a Time-of-Check Time-of-Use (TOCTOU) vulnerability where the target file gets improperly validated or overwritten.
**Learning:** Using `[ -f "$FILE" ] && [ ! -L "$FILE" ]` allows symlinks to fall through as a cache miss, allowing a malicious write to proceed.
**Prevention:** Explicitly abort script (`if [ -L "$FILE" ]; then abort; fi`) immediately before evaluating cache conditions on target output paths to prevent symlink traversal and overwrite attacks.
