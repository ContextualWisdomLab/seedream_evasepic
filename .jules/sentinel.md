...
## 2026-08-05 - Path Traversal in download script
**Vulnerability:** The `download-reference.sh` script allowed an attacker to supply a path containing directory traversal sequences (e.g., `../../../var/tmp/evil_video.mp4`) and write files outside the intended directory.
**Learning:** The script extracted the directory portion of the output path and created the directory without validating that the resulting path was within an intended safe directory.
**Prevention:** Implement input validation using `realpath` to resolve the absolute path and verify it is strictly within an allowed directory (e.g., `$PWD` or `/tmp`), being careful to match the prefix exactly by checking for trailing slashes (e.g. `$ABS_PWD/*`) to avoid bypassing.
