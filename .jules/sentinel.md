...
## 2026-08-26 - Prevent Arbitrary File Overwrite via Symlink cache bypass
**Vulnerability:** A script allowed a symlink to be passed as an output path. This could bypass a file cache check, leading to an Arbitrary File Overwrite vulnerability when the tool proceeds to write to it.
**Learning:** Using `[ -f "$FILE" ]` check handles symlinks transparently (if target exists), but it's important to not just append `[ ! -L "$FILE" ]` in the same check, which would just mean it's not a cache hit. You must explicitly check `if [ -L "$FILE" ]; then return 1; fi` and abort BEFORE checking cache or doing any writing.
**Prevention:** Use an explicit `[ -L ]` check and abort before any cache logic.
