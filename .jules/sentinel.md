...
## 2026-08-14 - [CRITICAL] 심볼릭 링크를 통한 임의 파일 덮어쓰기 취약점 수정
...
## 2026-08-14 - [CRITICAL] 캐시 확인 전 심볼릭 링크 명시적 검사를 통한 임의 파일 덮어쓰기 방지
...
## 2026-08-14 - [CRITICAL] 경로 조작(Path Traversal)을 통한 임의 디렉토리 파일 쓰기 방지
**Vulnerability:** 출력 경로($OUTPUT) 검증 누락으로 인해, `../` 와 같은 경로 탐색(Directory Traversal) 문자를 포함한 상대 경로를 악용하면 의도된 작업 디렉토리를 벗어나 시스템 상의 임의의 위치에 파일을 쓰거나 덮어쓸 수 있는 취약점(Path Traversal)이 발생함.
**Learning:** 파일 경로나 디렉토리를 다루는 Bash 스크립트에서는 전달받은 경로에 `..` 시퀀스가 포함되어 있는지 검증하지 않으면 상위 디렉토리로의 접근을 허용하게 됨.
**Prevention:** 출력 경로와 같은 사용자 입력값을 실제 파일 작업에 사용하기 전, Bash의 `case` 문 패턴 매칭(`*../*|*/..|..|*/../*`) 등을 활용해 경로 조작 문자가 포함되어 있는지 검사하고, 발견될 경우 에러와 함께 즉시 실행을 중단(`exit 1`)하도록 검증 로직을 추가해야 함.
## 2026-08-14 - [HIGH] 출력 디렉토리 심볼릭 링크를 통한 임의 파일 쓰기 취약점 방지
**Vulnerability:** 파일 출력 경로($OUTPUT) 자체의 심볼릭 링크를 검사하더라도, 해당 파일이 위치할 상위 디렉토리($OUT_DIR)가 공격자에 의해 심볼릭 링크로 조작된 상태라면, `mkdir -p`가 이를 따라가 목적지에 임의의 파일을 쓸 수 있는 취약점이 발견됨.
**Learning:** 파일 경로 검증 시 출력 대상 파일명 뿐만 아니라, 파일을 생성할 디렉토리 경로에 대해서도 `[ -L ]` 검사를 수행해야 디렉토리를 통한 심볼릭 우회 공격을 막을 수 있음. 부모 경로를 모두 검사할 수 없을 때에는 직접적인 목적지 상위 디렉토리(`$OUT_DIR`)에 대한 검사로 공격을 방어할 수 있음.
**Prevention:** 모든 상위 경로 구성 요소를 디렉토리 파일 디스크립터 기준으로 `O_NOFOLLOW`로 열고, 비공개 임시 디렉토리에 다운로드한 후 검증된 대상 디렉토리에서 노클로버 하드 링크로 원자적 게시한다. 게시 직전에 디렉토리 장치/아이노드 일치를 재검증하고 대상이 생겼거나 변경되면 실패 종료한다.
