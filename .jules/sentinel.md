...
## 2026-08-14 - [ANSI Escape Sequence Injection]
**Vulnerability:** Untrusted ffprobe metadata (duration, resolution, fps) was directly interpolated into bash printf "%b" format strings, allowing ANSI escape sequence injection.
**Learning:** Data from seemingly safe external tools like ffprobe (which parses untrusted media files) is fully user-controllable and can execute malicious terminal escapes.
**Prevention:** Always interpolate untrusted string variables using "%s", and restrict "%b" strictly to trusted ANSI color codes. Add all user-controlled bash variables to static grep tests.
