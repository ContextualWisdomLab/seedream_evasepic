...
## 2026-08-06 - [CRITICAL] Arbitrary File Overwrite via Symlink (TOCTOU)
**Vulnerability:** In `download-reference.sh`, checking cache existence using `[ -f "$OUTPUT" ] && [ -s "$OUTPUT" ]` and then later handing the same final pathname to `yt-dlp` allowed an attacker to replace that pathname with a symlink between validation and use. A pre-existing symlink could redirect writes immediately, and an entry-time `[ -L "$OUTPUT" ]` check alone still left a race window after the check.
**Learning:** Link rejection and TOCTOU closure are separate controls. A path property checked before an external command is not an identity guarantee for the later write. The first Sentinel repair correctly rejected a symlink already present at entry, but it remained incomplete because `yt-dlp` still opened the caller-controlled final pathname after the check.
**Prevention:** Keep the entry-time symlink rejection for fast, actionable failure, but never download directly to the final pathname. Download into a private unpredictable staging directory on the same filesystem, verify the staged result is a regular non-symlink file, then publish with a no-clobber hard-link operation. If any destination object appears during the download, fail closed without following or overwriting it and tell the caller to remove the unexpected path and retry. Keep an executable race regression that swaps the destination to a symlink after validation and verifies the protected target remains unchanged. See `docs/doctoring/download-publication-toctou.md` for CWE-59/CWE-367 traceability.

## 2026-08-29 - [MEDIUM] test_cli_ux.sh의 printf 인자 누락(ShellCheck SC2183) 수정
**Vulnerability:** `test_cli_ux.sh` 테스트 스크립트에서 에러 출력 시 `printf 'exit status: %s\n%s\n' "$PATH_TEST_OUTPUT"` 형태로 포맷 스트링은 `%s`를 두 개 기대하지만 인자는 한 개만 전달되어 ShellCheck SC2183 경고를 발생시켰습니다.
**Learning:** `printf` 포맷 스트링과 전달되는 인자의 개수가 일치하지 않으면 런타임에 의도치 않은 동작이나 보안 검사 도구(ShellCheck)의 실패를 유발합니다. 이전 커밋에서 `set -e` 정책을 개선하는 과정 중 실수로 변수 하나(`$PATH_TEST_STATUS`)가 삭제된 것이 원인이었습니다.
**Prevention:** 쉘 스크립트 작성 시 `printf`의 포맷 문자열과 변수의 개수가 정확히 일치하는지 확인하고, CI 파이프라인에서 `shellcheck`를 필수 단계로 두어 이러한 문법 및 포맷 오류가 병합되기 전에 차단되도록 해야 합니다.
