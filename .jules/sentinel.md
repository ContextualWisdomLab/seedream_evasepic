...
## 2024-05-18 - [ANSI Escape Sequence Injection]
**Vulnerability:** `plugins/seedream-evasepic/skills/analyze-reference-video/scripts/extract-frames.sh` 파일에서 `printf "%b\n" "${CYAN}Duration: ${NC}${DURATION}s | ${CYAN}Resolution: ${NC}$RESOLUTION | ${CYAN}FPS: ${NC}$FPS"` 형식으로 출력문을 사용하고 있었습니다. 신뢰할 수 없는 변수인 `$DURATION`, `$RESOLUTION`, `$FPS`가 `%b` 포맷 스트링을 통해 직접 보간되어, `ffprobe` 출력에 악의적인 문자가 포함된 경우 ANSI Escape Sequence Injection 취약점이 발생할 수 있었습니다. 이를 통해 공격자는 터미널 출력을 변조하거나 악성 코드 실행을 숨길 수 있습니다.
**Learning:** `printf "%b"`는 모든 인수에 대해 백슬래시 이스케이프를 해석합니다. 신뢰할 수 없는 데이터가 포함된 문자열을 포맷팅할 때 `%b` 지정자는 반드시 하드코딩된 ANSI 색상 코드에만 사용해야 합니다. 신뢰할 수 없는 데이터는 항상 `%s`로 출력해야 합니다.
**Prevention:** `printf` 포맷 스트링에서 항상 색상 코드와 신뢰할 수 없는 변수를 분리하십시오: `printf "%b%s%b\n" "\033[0;36mLabel: " "$UNTRUSTED_VAR" "\033[0m"`. 사용자가 제공하거나 외부에서 가져온 모든 값은 `%s`를 사용해야 합니다.
