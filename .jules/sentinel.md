...
## 2026-10-24 - Prevent symlink attacks on target output path
**Vulnerability:** The script used `[ -f "$OUTPUT" ] && [ -s "$OUTPUT" ]` as a cache hit condition. This allowed an attacker to pass a malicious symlink as the output path, which would be treated as a cache miss if the target was empty or missing, leading to arbitrary file overwrite when `yt-dlp` executes.
**Learning:** Checking for file existence before writing without verifying if the path is a symlink can lead to Time-of-Check to Time-of-Use (TOCTOU) symlink attacks.
**Prevention:** Explicitly abort (`if [ -L "$FILE" ]; then ...`) immediately before evaluating cache conditions on target output paths. Do not recursively reject symlinks in parent directories as it breaks benign system paths.
