...
## 2024-05-24 - Terminal output control vulnerability fix
**Vulnerability:** User-controlled inputs like video duration, resolution, and fps could contain unsanitized control characters such as terminal escape sequences that could inject or disrupt the console view during execution (Terminal injection/spoofing).
**Learning:** `printf "%b"` interprets escape characters in strings, and using it for variables generated dynamically or parsed from files directly allows injection if the file was modified by an attacker. Always sanitize strings using the provided `terminal_safe_text` functions.
**Prevention:** Avoid `printf "%b"` for any variable whose contents aren't exclusively trusted literals; apply shared neutralization logic like `terminal_safe_text` to format or strip controls explicitly. Add the unsafe variable identifiers to static tests enforcing this rule.
