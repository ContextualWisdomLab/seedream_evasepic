...
## 2024-06-01 - [ANSI Escape Sequence Injection in ffprobe metadata]
**Vulnerability:** External untrusted data (ffprobe metadata output like `$DURATION` and `$RESOLUTION`) was directly interpolated into a `printf "%b"` format string in `extract-frames.sh`. Because `%b` evaluates backslash escapes, malicious metadata containing `\033` could be executed as ANSI terminal escapes.
**Learning:** Metadata extracted from media files via external tools must be treated as untrusted user input, as it can be easily forged by attackers to include malicious payloads.
**Prevention:** Always separate the format string by using `%b` exclusively for trusted ANSI color codes and `%s` (or a terminal neutralizer) for printing untrusted inputs.
