...
## 2023-10-25 - [Prevent argument injection in command-line scripts]
**Vulnerability:** File path inputs passed to CLI scripts might start with a hyphen, causing underlying tools like `ffmpeg`, `whisper`, or `yt-dlp` to mistakenly parse them as command options.
**Learning:** Even though many tools try to parse positional arguments correctly or accept `--` before positional arguments, relying on this is unsafe. Also bash commands like `test -f "$FILE"` can incorrectly evaluate options like `-v`.
**Prevention:** Explicitly check if a file path starts with a hyphen (and is not an absolute path) and prepend `./` to neutralize it. Pattern: `[[ "$FILE" != /* ]] && [[ "$FILE" == -* ]] && FILE="./$FILE"`
