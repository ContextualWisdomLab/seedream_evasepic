...
## 2024-09-24 - [터미널 제어 문자 삽입 취약점]
**Vulnerability:** `ffprobe`를 통해 미디어 파일에서 추출한 메타데이터(duration, resolution, fps 등)를 신뢰할 수 없는 사용자 입력으로 간주하지 않고 터미널에 직접 출력하면, 파일에 악성 ANSI 이스케이프 문자가 포함되어 있을 경우 터미널 출력이 조작되거나 스푸핑(spoofing)될 수 있는 취약점이 있습니다.
**Learning:** 포맷 문자열 분리와 터미널 출력 무효화(neutralization)는 서로 다른 보안 제어입니다. 상호작용하는 터미널에서는 `%s`를 사용하는 것만으로는 충분하지 않습니다. 신뢰할 수 없는 모든 입력값은 반드시 제어 문자를 무효화하는 필터링을 먼저 거쳐야 합니다.
**Prevention:** 쉘 스크립트에서 검증되지 않은 데이터를 터미널에 출력할 때는 신뢰할 수 없는 모든 값을 먼저 `terminal_safe_text`에 통과시킨 다음, 반드시 `printf "%s"` 포맷을 사용하여 출력해야 합니다.
