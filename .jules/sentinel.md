...
**Vulnerability:** The `download-reference.sh` script had a cache check using `[ -f "$OUTPUT" ] && [ -s "$OUTPUT" ]`. If the path was an empty symlink, it bypassed the cache and allowed `yt-dlp` to overwrite the symlink target.
**Learning:** Adding `&& [ ! -L ]` to the cache condition causes symlinks to always be treated as a cache miss, leading directly to the overwrite vulnerability.
**Prevention:** Explicitly check for symlinks and abort using a non-zero 종료 상태 before evaluating any cache existence conditions.
