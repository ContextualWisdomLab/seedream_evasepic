...
## 2026-08-30 - Symlink Attack (TOCTOU) Mitigation in Bash Cache Checks
**Vulnerability:** A Time-of-Check to Time-of-Use (TOCTOU) vulnerability where `[ -f "$OUTPUT" ]` was used as a cache hit check for a download script (`download-reference.sh`), allowing an attacker to pre-create a symbolic link pointing to a sensitive file.
**Learning:** Standard bash checks like `[ -f "$FILE" ]` follow symlinks. If a symlink points to an existing file, the condition returns true. However, if the symlink is broken or the cache check fails for other reasons, subsequent write operations (like `yt-dlp -o "$OUTPUT"`) will overwrite the target of the symlink.
**Prevention:** Explicitly reject symlinks before any cache or existence checks for target output paths using `if [ -L "$OUTPUT" ]; then exit 1; fi`.
