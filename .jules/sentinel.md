...
## 2025-05-18 - Terminal Output Injection via ffprobe metadata
**Vulnerability:** The script `extract-frames.sh` used `printf "%b\n"` to directly interpolate untrusted video metadata (`DURATION`, `RESOLUTION`, `FPS`) retrieved from `ffprobe`. This allows Terminal Output Injection if a malicious video file contains embedded terminal control characters in its metadata fields.
**Learning:** Video metadata (like duration, resolution, fps) obtained from external tools like `ffprobe` is untrusted user input and must be treated just like user-provided URLs or paths. Format-string separation and output neutralization are required for all external data.
**Prevention:** Always route external metadata through `terminal_safe_text` and use `%s` (or `terminal_print_value`) for printing, keeping them separated from `%b` formatting directives used for trusted ANSI colors.
