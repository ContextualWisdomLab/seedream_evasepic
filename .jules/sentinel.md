...
## 2023-10-24 - ffmpeg 및 ffprobe 인자 삽입(Argument Injection) 취약점 수정
**Vulnerability:** `extract-frames.sh` 스크립트에서 `ffmpeg` 및 `ffprobe` 명령어 호출 시 사용자 입력을 보호하기 위해 `--` (end-of-options)를 사용했으나, 해당 도구들은 `--`를 지원하지 않아 입력 파일명으로 잘못 해석하는 취약점이 존재했습니다.
**Learning:** 모든 CLI 도구가 POSIX 표준인 `--` 옵션 구분자를 지원하는 것은 아닙니다. `ffmpeg`는 `--`를 무시하거나 입력 파일로 처리하여, 이후의 사용자 제어 파일 경로가 옵션이나 출력 대상으로 실행되는 심각한 인자 삽입 취약점을 유발할 수 있습니다.
**Prevention:** `ffmpeg`나 `ffprobe`처럼 `--`를 지원하지 않는 도구에 사용자 제어 파일 경로를 전달할 때는 경로가 하이픈(`-`)으로 시작할 경우 `./`를 접두어로 붙여 상대 경로로 명시적으로 변환(`[[ "$FILE" != /* ]] && [[ "$FILE" == -* ]] && FILE="./$FILE"`)하여 안전하게 처리해야 합니다.
