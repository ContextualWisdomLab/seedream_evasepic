...
## 2024-03-XX - Parent Directory Symlink Arbitrary File Overwrite 예방
**Vulnerability:** 파일 자체의 심볼릭 링크 여부만 검사하면 부모 디렉토리가 심볼릭 링크인 경우를 통과하여, 타겟 디렉토리에 원격 파일이 다운로드 및 덮어씌워지는 임의 파일 쓰기 공격에 노출될 수 있습니다.
**Learning:** 파일 작업을 수행하기 전에는 항상 최종 파일뿐만 아니라, 대상 경로의 모든 구성 요소(부모 디렉토리 포함)가 심볼릭 링크가 아닌지 재귀적으로 검사해야 합니다.
**Prevention:** 모든 디렉토리 경로 구성 요소를 분리하여 각 구성 요소에 대해 `[ -L ]` 검사를 수행하고, 심볼릭 링크가 발견되면 즉시 스크립트를 중단(Abort)하도록 구현해야 합니다.
