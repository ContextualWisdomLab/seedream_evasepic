...
## 2024-10-25 - Fix Symlink Arbitrary File Overwrite
**Vulnerability:** The script cached outputs using `-f "$OUTPUT"`, which followed symlinks and thus treated symlinks as cache misses, leading to `yt-dlp` overwriting the target file of the symlink (arbitrary file overwrite).
**Learning:** Checking for cache existence (`-f`) without ensuring the file is not a symlink first creates a Time-of-Check to Time-of-Use (TOCTOU) vulnerability where attackers can redirect file writes to sensitive locations.
**Prevention:** Always validate that an output file path is not a symlink (`if [ -L "$OUTPUT" ]; then exit 1; fi`) before performing cache checks or attempting to write to the file.
