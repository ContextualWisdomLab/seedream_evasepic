...
## 2026-07-21 - [CRITICAL] Fix arbitrary file overwrite via symlink
**Vulnerability:** The script used `[ -f "$OUTPUT" ] && [ -s "$OUTPUT" ]` for file cache check but did not check if the output was a symlink first. An attacker could pre-create a symlink at the output path pointing to an arbitrary file, leading to Arbitrary File Overwrite when the script downloads or processes data.
**Learning:** Symlinks must be explicitly rejected (`[ -L ]`) *before* applying existence checks (`[ -f ]`), as symlink validation combined in the same condition might lead to cache misses and subsequent overwrites.
**Prevention:** Explicitly check and abort if a symlink is detected (`if [ -L "$OUTPUT" ]; then exit 1; fi`) before performing cache existence or write operations.
