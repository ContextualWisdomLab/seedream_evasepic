...
## 2025-02-18 - ANSI Escape Sequence Injection via printf "%b"
**Vulnerability:** Untrusted media metadata (`$DURATION`, `$RESOLUTION`, `$FPS` from `ffprobe`) was interpolated into a `printf "%b\n"` format string in `extract-frames.sh`.
**Learning:** Using `printf "%b"` evaluates all backslash escapes in its interpolated arguments, making it vulnerable to ANSI escape sequence injection if an attacker crafts malicious metadata (e.g., `\033[0;31mPWNED\033[0m`).
**Prevention:** To safely mix ANSI terminal color codes with untrusted input in bash, strictly use `%b` in the format string exclusively for trusted ANSI control codes and use `%s` for printing untrusted user variables (e.g., `printf "%b%s%b\n" "${CYAN}Label: " "$VAR" "${NC}"`).
