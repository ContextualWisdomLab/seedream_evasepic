...
## 2026-09-05 - Prevent Symlink attacks on Output Paths
**Vulnerability:** A symlink in an output path could cause the script to overwrite arbitrary files when doing a cache check or writing files.
**Learning:** Checking for cache misses using `[ -f "$F" ]` isn't enough, as it allows writes to proceed through symlinks if the target file does not exist.
**Prevention:** Explicitly reject symlinks on target output paths before cache checks or writes.
