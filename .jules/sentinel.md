...
**Vulnerability:** In `download-reference.sh`, checking cache existence using `[ -f "$OUTPUT" ] && [ -s "$OUTPUT" ]` without verifying if `$OUTPUT` is a symlink allows an Arbitrary File Overwrite. If the target path is a symlink to an existing file, it's treated as a cache hit. But if the symlink target is empty or doesn't exist, the script proceeds and overwrites the symlink's target file using `yt-dlp`.
**Learning:** Using `[ -f ]` and `[ -s ]` will follow symlinks, leading to TOCTOU vulnerabilities. Just adding `&& [ ! -L ]` to the condition treats the symlink as a cache miss, making the vulnerability worse as the script continues to overwrite it.
**Prevention:** Explicitly check if the final target file is a symlink (`[ -L "$OUTPUT" ]`) and abort immediately (`exit 1`) before executing any further cache conditions or file operations.
