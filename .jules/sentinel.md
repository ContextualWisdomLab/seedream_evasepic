...
## 2026-09-24 - [HIGH] ffmpeg/ffprobe Option Injection
**Vulnerability:** `ffmpeg` and `ffprobe` commands in `extract-frames.sh` were susceptible to Option Injection if a user-provided `$VIDEO` path began with a hyphen (`-`). Using the `--` argument separator is not supported by these tools for the input file.
**Learning:** Utilities that do not conform to POSIX argument parsing guidelines (like `ffmpeg`, which parses `-i --` as an input file named `--`) require manual validation or sanitization of file paths to ensure they are not treated as options.
**Prevention:** If an external utility does not support the `--` end-of-options marker, prepend `./` to any file path that starts with a hyphen (e.g., `if [[ "$VIDEO" == -* ]]; then VIDEO="./$VIDEO"; fi`) to safely disambiguate it as a relative path.
