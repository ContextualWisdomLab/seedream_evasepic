...
## 2026-08-09 - [CRITICAL] ffprobe 메타데이터를 통한 터미널 출력 주입(Terminal Output Injection) 방지
**Vulnerability:** `extract-frames.sh` 스크립트에서 `ffprobe`로 추출한 동영상 메타데이터(FPS, Duration 등)를 검증이나 무효화(neutralization) 없이 `printf "%b"`를 사용하여 직접 터미널에 출력했습니다. 악의적으로 조작된 미디어 파일이 제어 문자나 ANSI 이스케이프 시퀀스를 반환할 경우 임의의 터미널 출력 변조 및 실행 가능성이 존재합니다.
**Learning:** 외부 도구(`ffprobe` 등)에서 추출된 모든 데이터는 신뢰할 수 없는 입력으로 간주해야 하며, 터미널 제어 문자를 렌더링할 수 있는 `%b`와 함께 사용되어서는 안 됩니다.
**Prevention:** 메타데이터 출력 시 `terminal_safe_text` 함수를 통해 제어 문자를 무효화하고, 서식 문자열과 데이터 변수를 명확히 분리하여 `%s` 포맷으로 안전하게 출력해야 합니다.
