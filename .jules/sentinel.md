...
## 2026-10-15 - [yt-dlp 설치 후 PATH 검증 누락]
**Vulnerability:** 스크립트가 `yt-dlp`를 자동 설치하려고 시도하지만, 설치 후 `$PATH`에서 해당 실행 파일을 다시 확인하지 않습니다. 이로 인해 'command not found' 같은 혼란스러운 실행 실패나 의도하지 않은 로컬 바이너리가 실행될 위험이 있습니다.
**Learning:** 의존성을 자동 설치한 후에는 반드시 설치된 실행 파일이 `$PATH` 시스템에 올바르게 반영되었는지 즉시 재검증해야 합니다. 접근할 수 없는 경우, 사용자에게 PATH를 수정하거나 수동으로 설치하라는 명확한 오류 메시지를 출력해야 합니다.
**Prevention:** 자동 설치 단계 이후에 항상 `command -v <tool>`을 사용하여 도구가 사용자 환경에 안전하게 등록되었는지 확인하십시오. 없을 경우 보안에 안전한 방식(오류 메시지 출력 후 종료)으로 처리합니다.
