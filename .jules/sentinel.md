...
## 2024-08-17 - [ffprobe 메타데이터 추출 시 터미널 ANSI 이스케이프 주입 취약점]
**Vulnerability:** `ffprobe`를 사용하여 미디어 파일의 메타데이터(duration, resolution, fps 등)를 추출하고 이를 `printf "%b"`를 사용하여 터미널에 출력할 때, 악의적으로 조작된 미디어 파일이 ANSI 이스케이프 시퀀스를 반환할 수 있어 터미널 제어 주입(Terminal escape injection) 취약점이 발생합니다.
**Learning:** 시스템 명령어(ffprobe)의 출력이 항상 안전할 것이라고 가정했습니다. 하지만 공격자가 미디어 파일의 메타데이터 필드를 조작하여 임의의 제어 문자를 삽입할 수 있으므로, 외부 도구에서 추출한 메타데이터 역시 신뢰할 수 없는 사용자 입력으로 취급해야 합니다. 또한, 신뢰할 수 없는 데이터는 `%b`가 아닌 `%s`를 사용하여 출력해야 합니다.
**Prevention:** 외부 도구에서 추출한 메타데이터 값을 터미널에 출력하기 전에 항상 `terminal_safe_text`와 같은 터미널 이스케이프 중화 함수를 사용하여 살균해야 하며, 안전한 ANSI 색상 코드와 섞어 출력할 때는 `%s` 포맷 지정자를 사용해야 합니다. 그리고 정적 분석 테스트 정규식(e.g., `test_terminal_output.sh`)에 새로 추가된 변수(`$DURATION`, `$RESOLUTION`, `$FPS` 등)를 포함하여 지속적으로 검증해야 합니다.
