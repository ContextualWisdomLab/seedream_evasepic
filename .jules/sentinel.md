...
## 2024-09-18 - 터미널 출력 주입 방지 및 메타데이터 살균
**Vulnerability:** `extract-frames.sh` 스크립트에서 `ffprobe`로 추출한 동영상 지속시간(DURATION), 해상도(RESOLUTION), 초당 프레임 수(FPS)를 터미널에 출력할 때 `terminal_safe_text`를 통해 메타데이터를 살균하지 않고 직접 출력했습니다. 이는 파일 메타데이터에 포함된 ANSI 이스케이프 시퀀스 등의 제어 문자를 통한 터미널 스푸핑(출력 조작)에 취약할 수 있습니다.
**Learning:** `ffprobe` 등으로 추출한 메타데이터도 사용자가 통제 가능한 신뢰할 수 없는 데이터로 취급해야 합니다.
**Prevention:** 쉘 스크립트에서 동적으로 생성되거나 추출된 모든 외부 메타데이터 값은 터미널에 출력하기 전에 `terminal_safe_text` 함수를 통과시키거나 `terminal_print_value`를 사용하여 잠재적인 제어 문자를 중화해야 합니다.
