...
## 2026-09-26 - Fix Command Option Injection Vulnerability
**Vulnerability:** Shell scripts passed untrusted input (file paths) directly to external binaries (like ffmpeg and whisper). If an attacker provided a path starting with a hyphen (e.g., `-someflag`), the binary would interpret it as an option, potentially leading to arbitrary command execution or system access.
**Learning:** External binaries do not always support the POSIX `--` end-of-options delimiter, leaving them susceptible to Option Injection if input begins with `-`.
**Prevention:** Always sanitize untrusted input used as file paths by prepending `./` if the input begins with a hyphen.
