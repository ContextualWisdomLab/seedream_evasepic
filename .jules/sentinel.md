...
## 2026-08-16 - [CRITICAL] Arbitrary File Overwrite via Symlink in File Cache Checks
**Vulnerability:** A cache check that uses `[ -f "$OUTPUT" ]` without validating `[ -L "$OUTPUT" ]` allows an attacker to supply a symlink as the target path. If the symlink points to a critical file but fails the cache check (e.g., zero-byte), downstream processes like `yt-dlp` will follow the symlink and overwrite the target file.
**Learning:** The `[ -f ]` check silently resolves symlinks. In environments where the user controls output file paths, failing to explicitly abort on symlinks can lead to Arbitrary File Overwrite vulnerabilities (TOCTOU). Mitigation should abort execution immediately rather than treating symlinks as cache misses.
**Prevention:** Always explicitly check for and reject symlinks using `if [ -L "$OUTPUT" ]; then exit 1; fi` before evaluating cache conditions or executing external commands that write to the output path.
