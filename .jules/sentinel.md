...
## 2026-08-09 - ffprobe metadata must use the shared terminal neutralizer
**Vulnerability:** `extract-frames.sh` separated trusted ANSI styling from ffprobe-derived metadata with `%s`, but raw ESC and other terminal control bytes embedded in `DURATION`, `RESOLUTION`, or `FPS` could still be interpreted by an interactive terminal.
**Learning:** `%s` prevents Bash from decoding backslash escapes; it does not neutralize control bytes that already exist in the value. External command output is untrusted at the terminal boundary even when that command normally emits numeric metadata.
**Prevention:** Pass every ffprobe-derived display value through `terminal_safe_text` before combining it with trusted color sequences. Keep an adversarial regression that injects an actual ESC byte from a stub ffprobe process and requires visible `\\x1B` output with no attacker-controlled raw escape sequence.
