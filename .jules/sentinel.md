...
## 2026-08-22 - TOCTOU File Cache Check Vulnerability
**Vulnerability:** In `download-reference.sh`, the file cache check `[ -f "$OUTPUT" ] && [ -s "$OUTPUT" ]` lacked a prior explicit symlink check (`[ -L ]`). This could lead to a Time-of-Check to Time-of-Use (TOCTOU) Arbitrary File Overwrite vulnerability if an attacker places a symlink before the script executes.
**Learning:** Adding `&& [ ! -L ]` to the same condition incorrectly treats symlinks as cache misses, continuing script execution and subsequently overwriting the symlink target. Symlinks must be treated as security violations and immediately abort execution.
**Prevention:** Implement explicit `if [ -L "$TARGET" ]; then exit 1; fi` checks prior to evaluating standard file existence cache logic.
