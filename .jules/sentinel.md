...
## 2024-05-24 - Untrusted ffprobe metadata rendering
**Vulnerability:** Extracted media metadata variables (duration, resolution, fps) from external tools like `ffprobe` were being injected directly into the terminal stream using `printf "%b"`, making the CLI vulnerable to ANSI escape sequence injection attacks if an attacker manipulated the media file's metadata tags.
**Learning:** Metadata extracted from media files via external tools must always be treated as untrusted user input, just like paths and URLs. When printing such neutralized data alongside trusted ANSI colors, the neutralized string must be interpolated using `%s` (e.g. `printf '%b%s' "$COLOR" "$SAFE_VAR"`). Passing neutralized text through `%b` re-evaluates the safe `\x` hex representations back into live control bytes.
**Prevention:** Explicitly pass all extracted metadata through a terminal escape neutralizer like `terminal_safe_text` before rendering, ensure the variables are added to static analysis grep checks, and always use `%s` for user-controlled strings in output commands.
