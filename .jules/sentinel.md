...
## 2024-05-24 - Unsanitized Metadata Rendered Without Terminal Neutralization
**Vulnerability:** Extracted video metadata (Duration, Resolution, FPS) obtained from `ffprobe` in `extract-frames.sh` was directly output to the terminal without being neutralized, which allows ANSI escape sequence injection if a malicious file mimics these properties.
**Learning:** Even internal tool output such as video metadata must be treated as untrusted user input when it depends on user-provided files, as it can be easily spoofed by malicious video files. In bash scripts, variables parsed from external tools must be explicitly neutralized via a helper function before being formatted into strings with trusted ANSI color codes.
**Prevention:** Always parse and sanitize the output of external media probes (like `ffprobe`) using `terminal_safe_text` before rendering them to standard output or standard error alongside ANSI colors, and strictly separate untrusted data strings (`%s`) from the interpreted ANSI sequences (`%b`).
