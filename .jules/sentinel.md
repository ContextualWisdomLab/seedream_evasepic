...
## 2026-08-06 - ffprobe 메타데이터를 통한 터미널 출력 인젝션(Terminal Output Injection) 방지
**Vulnerability:** `extract-frames.sh` 스크립트에서 `ffprobe`로 추출한 `DURATION`, `RESOLUTION`, `FPS` 메타데이터를 사전 무효화(neutralization) 없이 `printf "%b\n"` 포맷 문자열에 직접 보간하는 취약점이 있었습니다.
**Learning:** 비디오 메타데이터와 같이 파일에서 추출된 데이터는 신뢰할 수 없는 사용자 입력이며, ANSI 이스케이프와 같은 원시 터미널 제어 바이트를 포함할 수 있습니다. 이를 `%b`로 평가되는 포맷 문자열에 직접 전달하면 터미널 출력 인젝션이 발생합니다.
**Prevention:** 비디오 메타데이터와 같은 신뢰할 수 없는 데이터는 항상 전용 무효화 함수(예: `terminal_safe_text`)를 통해 정제하고, 신뢰할 수 있는 ANSI 색상 변수와 분리하여 `%s`를 사용해 포맷해야 합니다.
