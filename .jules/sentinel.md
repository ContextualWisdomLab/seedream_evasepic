...
## 2026-08-07 - Mitigate Subshell Performance Overhead in Utilities
**Vulnerability:** Bash functions performing critical sanitization like `terminal_safe_text` were invoked within subshells (e.g., `VAR="$(terminal_safe_text "$VAR")"`). Subshells incur significant process forking overhead, which can become a denial-of-service vector if executed repeatedly on large inputs.
**Learning:** Returning values via standard output from Bash functions forces the caller to use command substitution (subshells).
**Prevention:** Allow utility functions to accept an optional output variable name and use `printf -v` to assign the result directly to that variable in the current shell execution context, falling back to standard output only when omitted, avoiding unnecessary subshells.
