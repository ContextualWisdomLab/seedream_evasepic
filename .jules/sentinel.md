...
## 2025-08-25 - ANSI Escape Sequence Injection via FFprobe Metadata
**Vulnerability:** Extracted video metadata (duration, resolution, fps) from ffprobe was rendered directly in the terminal via `printf "%b"` without ANSI control byte sanitization.
**Learning:** Metadata extracted from media files via external tools like `ffprobe` must be treated as untrusted user input, as maliciously crafted files can inject ANSI sequences leading to terminal spoofing or command injection. When neutralizing, neutralized data must be printed with `%s` instead of `%b`.
**Prevention:** Always pass ffprobe metadata through `terminal_safe_text` and output using `%s` before combining with trusted color codes. Add corresponding variables to the static analysis regex test.
