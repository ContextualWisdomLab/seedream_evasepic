...
## 2026-09-23 - [CRITICAL] ffmpeg, ffprobe, whisper 명령어 옵션 파싱 시 -- 구분자 무시로 인한 경로 인수 주입 취약점
**Vulnerability:** `ffmpeg`, `ffprobe`, `whisper`와 같은 명령어에 파일 경로를 안전하게 전달하기 위해 `--` 옵션 구분자를 사용했지만(예: `-i -- "$VIDEO"`), 이 도구들은 `--`를 올바르게 처리하지 않아 하이픈(-)으로 시작하는 악의적인 파일 이름이 전달될 경우 인수 주입(Argument Injection) 취약점이 발생했습니다.
**Learning:** 모든 CLI 도구가 `--`를 POSIX 표준처럼 처리하지 않는다는 점을 확인했습니다. 특정 도구들은 입력 파일 앞에 `--`를 넣으면 해당 옵션을 무시하거나 파일로 인식하고, 이후에 오는 입력을 옵션으로 파싱해버리는 문제가 발생할 수 있습니다.
**Prevention:** `ffmpeg`, `ffprobe`, `whisper` 호출 시 `--` 구분자를 제거하고, 외부 입력 변수인 파일 경로에 대해, 하이픈(-)으로 시작하는 상대 경로일 경우 `./`를 강제로 붙이도록(예: `[[ "$FILE" != /* ]] && [[ "$FILE" == -* ]] && FILE="./$FILE"`) 사전 경로 정규화 방어 로직을 추가해야 합니다.
## 2026-09-23 - [CRITICAL] ffmpeg, ffprobe, whisper 명령어 옵션 파싱 시 -- 구분자 무시로 인한 경로 인수 주입 취약점
**Vulnerability:** `ffmpeg`, `ffprobe`, `whisper`와 같은 명령어에 파일 경로를 안전하게 전달하기 위해 `--` 옵션 구분자를 사용했지만(예: `-i -- "$VIDEO"`), 이 도구들은 `--`를 올바르게 처리하지 않아 하이픈(-)으로 시작하는 악의적인 파일 이름이 전달될 경우 인수 주입(Argument Injection) 취약점이 발생했습니다.
**Learning:** 모든 CLI 도구가 `--`를 POSIX 표준처럼 처리하지 않는다는 점을 확인했습니다. 특정 도구들은 입력 파일 앞에 `--`를 넣으면 해당 옵션을 무시하거나 파일로 인식하고, 이후에 오는 입력을 옵션으로 파싱해버리는 문제가 발생할 수 있습니다.
**Prevention:** `ffmpeg`, `ffprobe`, `whisper` 호출 시 `--` 구분자를 제거하고, 외부 입력 변수인 파일 경로에 대해, 하이픈(-)으로 시작하는 상대 경로일 경우 `./`를 강제로 붙이도록(예: `[[ "$FILE" != /* ]] && [[ "$FILE" == -* ]] && FILE="./$FILE"`) 사전 경로 정규화 방어 로직을 추가해야 합니다.
