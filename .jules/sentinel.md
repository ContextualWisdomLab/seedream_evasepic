...
## 2026-08-20 - [CRITICAL] Arbitrary File Overwrite via TOCTOU Symlink Attack
**Vulnerability:** The cache-hit check in `download-reference.sh` did not explicitly reject symlinks before checking file existence, allowing an attacker to create a symlink at the output path pointing to an arbitrary file, which would be overwritten by `yt-dlp`.
**Learning:** Checking for file existence and size `[ -f "$OUTPUT" ] && [ -s "$OUTPUT" ]` does not mitigate symlink attacks, as it resolves the symlink target. Adding `&& [ ! -L "$OUTPUT" ]` into the cache logic treats symlinks as cache misses, still leading to overwrite.
**Prevention:** Symlinks must be explicitly checked and rejected (`if [ -L "$OUTPUT" ]; then exit 1; fi`) immediately prior to evaluating cache hit conditions on target files.
