...
## 2026-08-16 - [CRITICAL] 경로 조작(Path Traversal)을 통한 임의 디렉토리 파일 쓰기 방지
...
## 2026-08-16 - [HIGH] 출력 디렉토리 심볼릭 링크를 통한 임의 파일 쓰기 취약점 방지
...
**Prevention:** 파일 생성 및 다운로드 로직 이전, 그리고 파일 존재 여부 캐시 확인 이전에 `OUT_DIR="${OUTPUT%/*}"` 를 구해 해당 상위 디렉토리 자체가 심볼릭 링크인지 검사(`if [ -L "$OUT_DIR" ]; then exit 1; fi`)하여 공격을 즉시 차단함.
