...
## 2026-09-25 - Option Injection Vulnerability in ffmpeg/ffprobe
**Vulnerability:** ffmpeg and ffprobe do not support `--` as an end-of-options delimiter. If a user provides a video filename that starts with a hyphen (e.g., `-input.mp4`), the CLI tools interpret the filename as a command-line option rather than an input file. This can lead to arbitrary option injection, allowing an attacker to read/write arbitrary files or execute code if advanced filters are used.
**Learning:** Command-line option parsers for tools like `ffmpeg` often deviate from POSIX guidelines, making standard safeguards like `--` ineffective against option injection attacks.
**Prevention:** Always manually sanitize user-provided file paths before passing them to tools that do not support `--`. Prepend `./` to any path that begins with a hyphen to ensure it is always treated as a relative file path rather than an option flag.
