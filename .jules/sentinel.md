...
**Learning:** 시스템 도구(`ffprobe`)에서 반환된 값이라도 그 출처가 악의적인 사용자가 제어 가능한 파일일 경우, 이를 신뢰해서는 안 됩니다.
**Prevention:** 쉘 스크립트에서 검증되지 않은 데이터를 터미널에 출력할 때는 반드시 `terminal_safe_text`와 같은 필터링 함수를 사용하거나 `printf "%s"` 포맷과 결합하여 제어 문자가 실행되지 않도록 무효화해야 합니다.
