...
## 2026-08-27 - Arbitrary File Overwrite via TOCTOU Symlink
**Vulnerability:** The `download-reference.sh` script passed the caller-controlled output path directly to `yt-dlp`. A symlink planted after an initial path check could therefore redirect the downloader and overwrite the symlink target.
**Learning:** A preflight symlink check is not atomic with a later write. Security-sensitive downloads must keep the network writer away from the caller-controlled destination and treat any destination mutation during the operation as a conflict.
**Prevention:** Download into a private staging directory, then publish to the requested destination with no-clobber creation semantics. Preserve non-empty cache hits, allow an existing zero-byte regular file to be replaced, and fail closed with an actionable retry message if any other destination appears before publish. Keep a regression test that plants the symlink while the downloader is running and verifies the target remains unchanged.
## 2026-08-31 - Post-Operation TOCTOU Symlink Race Condition
**Vulnerability:** Checking for symlinks only before an external process (like `yt-dlp`) runs is insufficient if the output path can be replaced with a symlink during execution. This allows an attacker to win a race condition and overwrite arbitrary files.
**Learning:** Pre-operation checks mitigate initial state vulnerabilities, but post-operation state changes can still introduce Arbitrary File Overwrite risks if the external tool follows symlinks created concurrently.
**Prevention:** Always re-validate the target file's integrity (e.g., explicitly ensuring it is not a symlink) immediately after the external command completes and before trusting or publishing the output.
