...
### 사용자 경험 개선 (UX)
- 참조 영상 CLI가 누락된 위치 인자를 이름으로 구분합니다. `<url>`, `<output_path>`, `<video_path>`, `<output_dir>`, `<audio_path>` 중 무엇이 빠졌는지 stderr에 표시하고, 기존 색상 Usage/Example을 유지한 채 종료 코드 2로 끝나므로 다음 명령을 바로 고칠 수 있습니다. 도구가 없는 PATH에서 종료 코드와 문구를 함께 잠급니다.
