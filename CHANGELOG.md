...
### 사용자 경험 개선 (UX)
- 참조 영상 다운로드 완료 시 파일 크기를 사람이 읽기 쉬운 형식으로 표시합니다. 1024 기반 스케일링에는 NIST/IEC 관례와 일치하는 `KiB`, `MiB`, `GiB`, `TiB`를 사용하고, 실행 가능한 CLI 회귀 테스트로 정확한 `1 MiB = 1,048,576 B` 표시를 고정합니다.
