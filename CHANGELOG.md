...
### 사용자 경험 개선 (UX)
- 참조 영상 다운로드가 끝나거나 캐시에 적중하면 파일 크기를 IEC 80000-13 이진 접두어(`KiB`, `MiB`, `GiB`)로 표시합니다. 1024 기반 환산에 SI 라벨(`KB`/`MB`/`GB`)을 쓰지 않으므로 2,097,152바이트 파일은 `2.00 MiB`로 보입니다. 바이트 수는 `PATH`가 아니라 `/usr/bin/wc` 또는 `/bin/wc`로 읽으므로, `yt-dlp` 없는 캐시 적중 PATH에서도 크기가 보입니다. 캐시된 파일이 실제 영상인지 다음 분석 전에 크기로 확인하세요.
